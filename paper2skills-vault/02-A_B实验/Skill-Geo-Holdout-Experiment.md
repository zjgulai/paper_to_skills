...
**三轨验证**：
- **成本**：需购买Nielsen DMA级销售数据（约$5,000-15,000/年），或自建ERP地区汇总；协调广告平台按DMA分区投放需额外人力（约2-3人周）；计算成本极低（单次实验< $10云资源）
- **合规**：不触碰GDPR（仅聚合数据，无个人标识）；不违反Amazon政策（非价格测试）；需注意广告平台服务条款中是否禁止"控制组不投放"（如Meta允许Brand Lift实验，TikTok需申请白名单）
- **风险**：控制组完全不投广告可能导致该地区市场份额短期流失给竞品（尤其长周期实验）；若实验泄露可能引发经销商不满（"为什么我们区域没有广告支持"）；建议控制组投放最低维持量而非零投放

**场景B：亚马逊站内Deal效果评估（按国家/州）**
- 业务问题：亚马逊美国站计划对婴儿推车做Lightning Deal，但无法对同一ASIN做用户级A/B（违反ToS），需要评估Deal的真实增量
- 数据要求：按州级历史销售数据（12周前期）+ Deal排期表
- 预期产出：选择10对相似州（如加州vs德州，纽约vs佛州），处理组州做Deal，控制组州不做。Geo DiD估计Deal真实增量 = +15.2%（区间[8.7%, 21.7%]），而亚马逊后台"Deal效果报告"显示+35%（高估19.8pp）
- 业务价值：发现Deal的"虚假繁荣"（大部分销量来自原本就会购买的顾客），优化Deal频率和折扣力度，年化节省约25万元折扣成本

**三轨验证**：
- **成本**：需获取亚马逊州级销售数据（通过Vendor Central或第三方工具如Helium10，约$1,000-3,000/月）；协调Deal按州开启需与亚马逊客户经理沟通（部分Deal不支持区域限制，需改用Coupon按州投放）；计算成本极低
- **合规**：严格遵守Amazon ToS（不进行价格A/B测试，仅评估Deal效果）；不违反GDPR（无个人数据）；需注意美国各州广告法差异（如加州CCPA要求更严格的数据处理披露）
- **风险**：控制组州消费者可能通过跨州购买获取Deal（漏出效应），导致效应低估；若竞品监测到实验，可能在控制组州发起针对性促销，污染实验结果；建议在实验期间监控跨州物流数据

**场景C：独立站SEO策略评估（按城市）**
- 业务问题：母婴独立站计划优化"婴儿推车"品类页的SEO（标题/描述/结构化数据），想验证是否带来真实增量搜索流量和转化
- 数据要求：按城市级Google Search Console数据（16周前期）+ 部署方案
- 预期产出：选择20个配对城市（按搜索量/转化率相似），处理组城市部署SEO优化，控制组维持原样。Geo DiD估计SEO优化带来的真实增量 = +12.3%（区间[6.8%, 17.8%]），而Google Search Console显示+28%（高估15.7pp，因包含品牌词增长）
- 业务价值：准确衡量SEO投入的ROI，避免过度投资于低效关键词，年化节省约15万元SEO外包费用

**三轨验证**：
- **成本**：需城市级搜索数据（Google Search Console免费但需按URL分组，或使用Ahrefs/SEMrush城市级数据，约$500-2,000/月）；SEO部署需开发资源（约1-2人周）；计算成本极低
- **合规**：符合Google Webmaster Guidelines（非黑帽SEO）；不违反GDPR（聚合搜索数据）；需注意Google可能将同一用户在不同城市的搜索归因到不同城市，造成数据污染
- **风险**：SEO优化可能被Google算法更新干扰（如核心更新期间实验失效）；控制组城市用户可能通过VPN访问处理组页面，导致漏出；建议在实验期间监控Google算法更新公告，必要时延长实验期
