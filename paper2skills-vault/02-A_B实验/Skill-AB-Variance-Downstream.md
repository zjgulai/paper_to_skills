...
**三轨验证**：
- **成本**：LLM 嵌入调用费用约 $0.002/次（text-embedding-3-small），800 样本约 $1.6；额外开发工时 2 天（集成 SDK + 回归模块）
- **合规**：用户历史行为数据需脱敏（去除 PII），符合 GDPR 第 5 条数据最小化原则；Amazon 未禁止使用外部 LLM 处理行为数据，但需确保不违反《Amazon 数据保护政策》第 3.2 条
- **风险**：LLM 嵌入可能引入隐性偏见（如对高消费用户群体过拟合），导致实验结论偏向特定人群；建议分层验证协变量分布
...
**三轨验证**：
- **成本**：视频文案转录 + LLM 嵌入费用约 $0.01/素材（Whisper + embedding），单次测试 2 个素材 × 500 用户 = $10；人力成本 1 天（配置自动化流水线）
- **合规**：TikTok 广告 API 禁止未经用户同意抓取互动数据（违反《TikTok 开发者协议》第 5.2 条）；需使用平台授权的 Audience Insights 数据，或用户明确授权的第一方数据
- **风险**：若协变量包含敏感属性（如年龄、性别），可能触发《广告法》第 9 条"不得利用算法歧视消费者"；建议仅使用行为特征（点击率、观看时长）而非人口统计特征
