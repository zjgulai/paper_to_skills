...
**三轨验证**：
- **成本**：显性成本包括云服务器（约 ¥3,000-5,000/月用于事件流处理+数据库）、开发人力（约 3-4 周，折合 ¥2-3 万）、第三方分析工具（如 Mixpanel/Amplitude 约 ¥1,000-2,000/月）。总首年成本约 ¥8-12 万。
- **合规**：需确保用户 ID 匿名化处理（不存储 PII）；实验数据收集需符合 GDPR/CCPA 要求（提供 opt-out 机制）；Amazon 平台禁止对搜索结果排名进行人为干预实验，价格实验需避免触发“操纵定价”嫌疑。
- **风险**：① 实验冲突未完全消除（同层实验仍可能相互污染）；② 统计显著性误判（多重比较未校正导致假阳性）；③ 负面实验结果泄露（如价格敏感度数据被竞品获取）；④ 平台审查风险（Amazon 可能将大规模实验视为操纵行为）。
