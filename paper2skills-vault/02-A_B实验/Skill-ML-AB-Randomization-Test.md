...
- **三轨验证**：
  - **成本**：需额外采集用户类型、设备、时段等协变量数据（已有埋点，零新增成本）；ML模型训练与排列检验需约2小时/次（单台8核服务器，成本<10美元）
  - **合规**：不触碰Amazon政策红线（仅基于自有站内数据）；GDPR合规需确保用户ID匿名化处理，不涉及敏感个人信息
  - **风险**：低风险。个性化页面可能导致新用户感知到"区别对待"，需在文案上统一品牌调性；无价格战或平台审查风险
...
- **三轨验证**：
  - **成本**：需构建用户社交网络（基于分享/推荐/共同购买数据），数据清洗与图构建约5人天；每次检验需额外计算邻居特征（约3小时/次，GPU可选）
  - **合规**：社交网络数据需确保用户同意条款覆盖"行为关联分析"；Amazon政策禁止操纵库存感知，但本方法仅用于检测，不涉及干预
  - **风险**：中等风险。若检测到强干扰效应，需调整实验设计（如网络分区随机化），可能延迟大促上线时间；需避免公开讨论"干扰效应"以免引发平台对库存机制的审查
...
- **可组合（combinable）**：[[Skill-Bayesian-AB-Testing]]（贝叶斯自适应实验+ML随机化检验联合提升实验效率）、[[Skill-SOP-Sales-Operations-Planning]]（实验结果驱动运营计划更新）
