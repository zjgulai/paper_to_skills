...
**三轨验证**：
- **成本**：数据采集需埋点追踪推荐来源（约 2 周开发）；计算资源极低（单机 Python 即可）；人力成本约 1 名数据分析师 3 周。
- **合规**：Amazon 允许推荐计划但禁止"刷单"或虚假评价；GDPR 下需明确告知用户推荐数据用途；中国广告法禁止"拉人头"式传销结构（需确保奖励层级 ≤ 2 级）。
- **风险**：奖励过高可能引发"羊毛党"批量注册；竞品可能跟进导致奖励成本通胀；若推荐文案夸大宣传，可能触发平台审查或品牌信任危机。
