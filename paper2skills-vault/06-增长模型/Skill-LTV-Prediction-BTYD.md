...
**三轨验证**：
- **成本**：数据采集成本低（仅需订单表，无需额外埋点）；计算资源消耗小（单次建模 < 1小时，5000用户）；人力成本约 1 人周（数据清洗+模型调优+结果解读）。
- **合规**：不触碰 Amazon 政策红线（仅使用自有订单数据，不涉及排名/评论操纵）；GDPR 合规需确保 customer_id 脱敏（使用哈希 ID）；不涉及广告法敏感词。
- **风险**：激活邮件频率过高可能触发 Amazon 邮件政策限制（每日上限）；对"沉睡"用户过度优惠可能养成"等券再买"习惯，降低自然转化率；模型误判（将真流失用户标记为沉睡）导致营销预算浪费（约 5-10% 误判率）。
...
**三轨验证**：
- **成本**：UTM 参数部署成本极低（约 0.5 人天）；Amazon Attribution 需付费订阅（$500-2000/月）；模型计算成本同场景A；人力成本约 0.5 人周。
- **合规**：Amazon Attribution 数据使用需遵守 Amazon 广告政策（不得跨账户合并数据）；GDPR 下渠道归因需用户同意 cookie（EU 用户）；不得将 CLV 数据用于再营销受众定向（违反平台政策）。
- **风险**：渠道归因偏差（首次点击 vs 末次点击模型差异大）；TikTok 渠道用户 CLV 可能被高估（年轻用户生命周期短）；预算重分配可能导致单一渠道依赖度过高（>60%），增加供应链/政策风险；竞品可能通过监测投放变化反向推断策略。
