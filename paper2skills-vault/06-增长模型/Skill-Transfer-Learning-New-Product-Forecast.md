...
**三轨验证**：
- **成本**：数据采集需整合 ERP/SCM 中所有相似品 52 周历史（约 0.5 人月）；计算资源单次预测 < $5（CPU 即可）；人力成本主要在特征工程（约 2 人周）
- **合规**：使用内部历史数据，不涉及 Amazon 实时销售数据抓取，无 GDPR 风险；预测结果仅用于内部备货决策，不触碰广告法
- **风险**：若相似品选择偏差（如误将清仓品当爆款），可能导致备货过量；建议设置 P90 上限阈值 + 人工复核 Top-3 相似品逻辑
...
**三轨验证**：
- **成本**：需获取 US 站完整销售历史（内部数据免费）；德国站本地化特征需采购第三方数据（育儿偏好评分约 $2000/年）；模型适配约 1 人月
- **合规**：跨市场数据迁移需确认 GDPR 合规（US 数据不可含 EU 用户 PII）；德国站需 CE 认证标识，预测模型不涉及产品合规声明
- **风险**：US→EU 迁移存在市场差异风险（如德国消费者对价格敏感度不同），建议在 fine-tune 阶段加入至少 2 周德国站早期销售数据做校准；汇率波动可能导致备货成本偏差
