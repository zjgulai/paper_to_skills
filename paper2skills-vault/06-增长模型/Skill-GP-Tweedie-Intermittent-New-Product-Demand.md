...
- **三轨验证**：
  - **成本**：数据采集成本低（仅需内部销售记录）；计算资源需求中等（MCMC 推断需 GPU 或云实例，单 SKU 约 0.5-2 小时）；人力成本约 0.5 人月（模型调参 + 部署）
  - **合规**：仅使用内部交易数据，不涉及用户个人信息，无 GDPR/CCPA 风险；不触碰 Amazon 定价或广告政策
  - **风险**：若竞品同步采用类似模型，可能加剧备货竞赛导致行业库存过剩；高分位数备货策略在需求骤降时可能产生滞销库存；需设置 P90 上限阈值防止过度备货
...
- **三轨验证**：
  - **成本**：数据采集成本低（内部销售 + 品类属性）；计算资源需求低（轻量版无需 GPU，单品类约 10 分钟）；人力成本约 0.3 人月（模板化部署）
  - **合规**：无用户数据使用，完全合规；FBA 库存策略符合 Amazon 仓储政策
  - **风险**：小众品类需求波动大，P90 分位数可能仍低估极端事件（如社交媒体爆红）；过度依赖历史稀疏数据可能导致模型漂移；需设置人工干预机制（如销量突增 5 倍时触发人工复核）
