...
**成本轨**
- 数据采集：评论爬取/API调用成本 $200/月（Amazon Product API或第三方数据源）
- 计算资源：词典匹配+Sigmoid计算，单次处理380条评论<100ms，云函数成本$50/月
- 人力投入：初期词典构建与权重校准 40小时（$2,000），后续月度维护 4小时（$200/月）
- **月度总成本**：$450/月（$5,400/年），相比NPS调研$3,000/次×12次=$36,000/年，成本节省85%

**合规轨**
- ✅ **Amazon政策**：评论数据来自公开平台，符合Amazon ToS第4.2条（数据使用范围内）；邮件营销需遵守CAN-SPAM法案（已含unsubscribe链接）
- ✅ **GDPR**：评论文本已脱敏处理（不含用户ID/邮箱），符合GDPR第5条数据最小化原则
- ✅ **广告法**：邮件中「第二件8折」需标注"仅限新客"或"限购X件"，避免虚假宣传；中国《反不正当竞争法》第8条禁止虚假宣传
- ✅ **跨境贸易**：评论语言特征提取不涉及个人隐私分类，符合各国数据出口规范
- **风险等级**：低风险，无合规红线

**风险轨**
- **竞品价格战**（概率15%）：若邮件中折扣过激（>30%），可能引发竞品跟风降价，导致行业毛利下滑。*缓解*：折扣幅度与NPS_proxy挂钩，NPS<50时禁用折扣，改用赠品策略
- **平台审查**（概率8%）：Amazon可能因邮件频率过高（周发）触发垃圾邮件审查。*缓解*：邮件频率改为双周发送，或通过站内信替代
- **品牌损伤**（概率5%）：若NPS_proxy预测误差大（实际留存30%但预测60%），大规模发送折扣邮件到不满意用户，可能引发负面评论反弹。*缓解*：预测值<50%时人工审核，或结合产品评分(rating)二次确认
- **数据泄露**（概率3%）：评论文本存储在云端，若加密不足可能被黑客利用。*缓解*：启用AES-256加密+访问日志审计，成本+$100/月
- **综合风险等级**：中低风险，年度损失期望值 <$5,000（基于概率×影响评估）
