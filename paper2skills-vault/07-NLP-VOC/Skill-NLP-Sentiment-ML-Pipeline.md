...
**三轨验证**：
- **成本**：显性成本约 ¥3-8 万/年。包括：评论数据采集（爬虫或 API 订阅 ¥1-2 万/年）、NLP 情感分析计算资源（云服务器 ¥0.5-1 万/年）、数据管道维护人力（0.5 人月/季度，约 ¥1.5-3 万/年）。若使用第三方情感分析 API（如 AWS Comprehend），额外增加 ¥0.5-2 万/年。
- **合规**：需注意三条红线：① Amazon 评论爬取违反 ToS，必须通过 Amazon Selling Partner API（SP-API）或授权第三方工具获取；② 评论数据含用户个人信息（如用户名），需脱敏处理以符合 GDPR/CCPA；③ 情感特征不得用于价格歧视（如根据用户评论情感动态调整个性化定价），违反欧盟《数字服务法》和美国 FTC 指南。
- **风险**：主要风险包括：① 情感信号滞后（如评论审核延迟导致信号晚于销量变化），造成模型误判；② 竞品通过刷评操纵情感特征，导致预测偏差；③ 过度依赖情感特征可能忽视结构性市场变化（如竞品新品上市），建议保留传统特征作为基线。
