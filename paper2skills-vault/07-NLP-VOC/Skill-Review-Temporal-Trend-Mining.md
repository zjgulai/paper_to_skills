...
- **三轨验证**：
  - **成本**：数据采集成本低（Amazon 后台直接导出 Review CSV）；计算资源约 0.5 元/ASIN/月（单台云服务器可处理 100+ ASIN）；人力投入约 2 小时/次（模型调参与结果解读）
  - **合规**：不触碰 Amazon 政策红线（仅使用公开 Review 数据，不涉及刷评或操纵排名）；不涉及 GDPR 问题（Review 为公开数据，不收集用户个人信息）；无广告法风险
  - **风险**：若误判趋势（如季节性波动被识别为质量问题），可能导致不必要的产品召回或供应商更换，造成约 5 万元误判成本；建议结合人工复核确认后再行动
...
- **三轨验证**：
  - **成本**：数据采集需通过第三方工具（如 Helium10、Jungle Scout）获取竞品 Review，月费约 300-500 元/工具；计算资源同上；人力投入约 3 小时/次（含竞品选择与报告解读）
  - **合规**：合规风险较低（使用公开数据，不涉及爬虫或数据窃取）；但需注意 Amazon 对竞品监控的容忍度——若大规模自动化监控并用于恶意差评或虚假举报，可能触发平台审查；建议仅用于内部决策，不对外公开
  - **风险**：若竞品发现我方针对性广告投放，可能引发价格战或广告竞价升级，导致 ACOS 上升 10-15%；建议配合差异化策略（如强调自身优势而非攻击竞品弱点）降低对抗风险
...
- **可组合（combinable）**：[[Skill-Epidemiological-Viral-Traffic-SIR]]（评论趋势可视作传播曲线的早期信号）
## ⑤ 商业价值评估
