...
| 子目录 | 内容 | 状态 |
|--------|------|------|
| `tag-evolution/` | **标签进化工作流（Phase 1~3）** — 统一萃取、逆向分析、覆盖率提升 | ✅ 已完成 |
| `data-sampling/` | Reddit/Trustpilot/Zendesk 数据采样脚本 | ✅ 已完成 |
| `batch-labeling/` | 批量打标入口脚本 | ✅ 已完成 |
| `nps-pipeline/` | NPS 校准、看板生成流程脚本 | ✅ 已完成 |
| `diagnostics/` | 标签诊断、覆盖率测试、快速验证脚本 | ✅ 已完成 |
| `data-processing/` | 标签词典扩展、品线矩阵生成、关键词挖掘 | ✅ 已完成 |

---

## tag-evolution/ 子目录（核心）

标签进化工作流是 VOC 萃取体系的核心迭代引擎，覆盖数据统一 → 萃取打标 → 逆向分析 → 字典更新 → 覆盖率提升的完整闭环。

详见 [tag-evolution/README.md](tag-evolution/README.md)。
