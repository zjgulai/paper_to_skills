...
- 主责标签：v4.0 字典 `主责部门=品控部` 共 14 个标签
...
- 推荐行动：见 [maa_strategy_generator --dept quality_control](../04-输出结果/10-周报/2026-W19/品控部.md)
```
...
| KPI | 定义 | 单位 | 阈值（黄/红）|
|---|---|---|---|
| 安全/合规标签命中量 | 字典 `主责部门=质量与法规部` 标签合计 | 条 | > 5 / > 15 |
...
- 主责标签：v4.0 字典 `主责部门=质量与法规部` 共 8 个标签
...
- 合规风险事件：0 起召回（健康）
- BPA / 化学成分提及 7 条（绿色）
- FDA 相关咨询 3 条（路由至客服 + 法务）
- 推荐行动：见 [maa_strategy_generator --dept regulatory](../04-输出结果/10-周报/2026-W19/质量与法规部.md)
```
