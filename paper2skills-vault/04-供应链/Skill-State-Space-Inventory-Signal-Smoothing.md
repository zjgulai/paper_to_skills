...
- **三轨验证**：
  - **成本**：数据采集依赖亚马逊历史报告（免费），计算资源单SKU约0.01元/次（AWS Lambda），人力投入约2人天/月（参数维护+异常监控），总成本极低。
  - **合规**：仅使用亚马逊官方提供的销量/库存报告，不涉及用户隐私数据，符合Amazon Seller Central数据使用条款及GDPR要求。
  - **风险**：季节周期误判（如将一次性促销脉冲误认为季节模式）可能导致备货偏差；建议每季度人工复核季节因子，避免模型漂移。
...
- **三轨验证**：
  - **成本**：需额外采集促销日期/折扣数据（可从广告报表或内部活动日历获取），计算量增加约30%（需扩展状态向量），人力投入约1人天/次大促（数据标注+结果审核）。
  - **合规**：促销数据为内部运营数据，不涉及消费者个人信息；若使用第三方折扣追踪工具，需确保工具符合Amazon API使用政策。
  - **风险**：促销效应与季节效应可能存在共线性（如Prime Day恰逢销售旺季），导致分解不稳定；建议引入折扣幅度作为协变量，并设置促销效应衰减系数（促销后2周内逐步归零）。
