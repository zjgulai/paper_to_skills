...
**三轨验证**：
- **成本**：数据采集需接入多SKU历史销量（含关联购买记录）及供应商Lead Time分布，约需2-3周数据清洗；计算资源需GPU训练（如AWS p3.2xlarge约$0.9/小时），训练周期约3-5天；人力投入包括数据工程师+算法工程师各1名，约4-6周。
- **合规**：不直接触碰Amazon政策红线（补货决策为内部运营行为）；不涉及GDPR/广告法；但需注意训练数据中若包含用户购买关联记录，需确保脱敏处理（不涉及个人身份信息）。
- **风险**：训练初期策略不稳定可能导致短期缺货或过度补货，需设置安全回退机制（如并行运行启发式规则作为兜底）；若DRL策略过度优化单一SKU（如吸奶器），可能引发其他SKU库存失衡，需定期人工审计。
