...
- **三轨验证**：
  - **成本**：需接入FBA日库存API（约¥2000/月数据费）+ 1名运营每周30分钟调参，年化成本约¥3.5万
  - **合规**：仅使用自有销售与库存数据，不触碰Amazon用户隐私；不涉及价格操纵，符合Amazon库存管理政策
  - **风险**：若Kd参数过大，可能在需求短期波动时过度上调安全库存，导致旺季前资金挤占；建议每两周人工复核一次控制信号分解
...
- **三轨验证**：
  - **成本**：需额外标记促销期事件（人工成本约¥500/次大促），无新增数据采购
  - **合规**：促销期标记为内部运营数据，不涉及消费者隐私；安全库存调整不影响标价，不触发价格合规审查
  - **风险**：D项对需求下降的响应可能过于灵敏，若促销后需求快速回升（如返校季补货），可能误判为持续低谷导致备货不足；建议对D项输出设置±20%的限幅
