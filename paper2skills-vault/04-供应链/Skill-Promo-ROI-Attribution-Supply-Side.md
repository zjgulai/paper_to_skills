...
**三轨验证**：
- **成本**：需从ERP/仓储系统提取备货时间戳、库存周转记录、退货处理工单，数据清洗约需2人天；计算资源可忽略。
- **合规**：不涉及用户隐私或广告法；需确保清仓折扣不违反Amazon MAP（最低广告价格）政策，避免因大幅折价触发平台价格垄断审查。
- **风险**：若公开披露“真实ROI”数据，可能被竞品反向推算成本结构；建议仅内部使用，不对外发布。
...
**三轨验证**：
- **成本**：需建立统一的数据口径（如备货提前天数、尾货定义），跨期对比需额外1人天对齐数据源。
- **合规**：对比结果若用于对外宣传（如“618比黑五更赚钱”），需确保数据脱敏，避免泄露具体GMV或成本绝对值，违反上市公司信息披露规则。
- **风险**：纵向对比可能暴露季节性备货策略缺陷，若被内部团队误读为“砍掉黑五备货”，会导致旺季断货；需配套解释“规模效应与风险分散”的平衡。
