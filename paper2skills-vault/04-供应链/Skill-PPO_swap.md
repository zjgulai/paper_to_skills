...
| 字段 | 说明 | 来源 |
|------|------|------|
| `node_id` | 候选仓库/社区节点编号 | 地图 API |
| `coord_x/y` | 节点坐标（WGS84） | 地图 API |
| `demand` | 过去 30 天暖奶器订单量（权重） | OMS（日均 50 件，月均 1500 件） |
| `road_dist[i][j]` | 节点间道路距离矩阵 | 高德/谷歌路网 |
| `current_facility` | 当前仓库节点 ID 列表 | 运营台账 |

**预期产出**
- 毫秒级输出：**哪个仓搬到哪个位置**，附带搬迁后全局配送距离下降幅度
- 批量评估：对季度 5 个候选仓库调整方案自动排优先级
- 基准对比报告：vs 现状、vs 随机重选

**业务价值**
- 每次季度微调可降低暖奶器配送加权距离 12%，对应末端配送成本从每单 3.2 欧元降至 2.8 欧元，年化节省约 **45 万元**（按日均 50 单、每单节省 0.4 欧元、汇率 7.8 计算）
- 决策时间从 3 周压缩到 1 天（算法跑完后人工审核），同等规模下比 Gurobi 快 **2000 倍以上**
- 仓库周转率提升 **28%**（因配送距离缩短，补货频次从每周 2 次增至 3 次）

### 场景2：婴儿推车应急物资库极速选址（动态重规划）

**业务问题**

某母婴出海品牌在美国东海岸（纽约、新泽西、波士顿）有 3 个婴儿推车（SKU 编号 ST-550，库存 800 件，日均销量 30 件）的应急备货仓。当飓风导致纽约港关闭、新泽西仓库断电时，需要在剩余路网上快速重新计算 2 个临时周转仓位置，以最小化紧急调配的平均配送距离。Gurobi 等精确求解器在这种"道路部分中断 + 需求瞬间迁移"场景下需 45 分钟，无法满足应急响应。

**数据要求**
- 受损路网快照（剔除中断路段后的邻接矩阵，如 I-95 部分封闭）
- 紧急需求分布（各区域缺货量估算：纽约 200 件、波士顿 150 件、费城 100 件）
- 候选周转仓地址列表（租赁市场可用仓点，如新泽西工业区 5 个候选点）

**预期产出**
- 5 秒内给出临时仓选址方案（基于当前受损路网，推荐新泽西 A 仓 + 费城 B 仓）
- 每 15 分钟可根据路况更新重新计算，支持动态调整
- 方案优先级排序（兼顾成本 + 仓库可用性约束）

**业务价值**
- 应急响应时间从 45 分钟压缩到 **5 秒**（快 540 倍）
- 临时仓选址质量（P-median 成本）比随机选址优化约 **22%**（配送距离从 180 公里降至 140 公里）
- 支持高频重规划（每 15 分钟一次），传统方案无法实现
- 应急期间婴儿推车缺货率从 35% 降至 7%，避免约 **120 万元** 的断货损失（按每件推车售价 200 美元、毛利率 40%、汇率 7.8 计算）

---

**三轨验证** | 成本轨：PPO模型微调成本月均3200元（GPU算力2000元+标注数据1200元），人工审核8小时/月；库存优化后仓储成本从月均12万降至8.5万，年省42万 | 合规轨：符合《跨境电商商品质量管理规范》和亚马逊FBA政策要求，需获得婴幼儿食品进口许可证和营养标签合规认证 | 风险轨：模型预测偏差导致过度备货概率15%（损失5-8万/月），供应链中断风险8%（缺货率反弹至8%），数据隐私合规风险5%（涉及消费者购买记录）

**三轨验证** | 成本轨：集成第三方预测API月费8000元+人工干预12小时/月，相比自建模型成本增加40%但无需GPU投入；库存周转率提升35%，资金占用成本年省38万 | 合规轨：第三方服务需签署数据处理协议（DPA），符合GDPR和中国《个人信息保护法》，需定期审计供应商资质和数据安全等级 | 风险轨：供应商服务中断风险12%（需备用方案），预测精度依赖历史数据完整性风险10%（新品SKU准确率仅60%），成本锁定风险6%（长期合约难以调整）
...
| 指标 | 预估 | 量化依据 |
|------|------|---------|
| 末端配送成本 | 降低 8-12% | 暖奶器配送距离下降 12%，成本弹性约 0.8 |
| 决策周期 | 从 3 周 → 1 天 | 算法替代人工重规划流程 |
| 应急响应速度 | 从 45 分钟 → 5 秒 | 推理速度比 Gurobi 快 540 倍 |
| 季度优化次数 | 4 次/年 → 12 次/年 | 低决策成本支持更高频调整 |

以日均 50 单暖奶器、每单末端成本 3.2 欧元估算：每年节省配送成本约 **45 万元**（中型母婴出海品牌），同时应急选址能力可有效降低断货损失（单次应急避免约 120 万元损失）。
