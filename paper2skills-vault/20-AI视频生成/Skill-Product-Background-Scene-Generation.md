...
**三轨验证**：
- **成本**：显性成本包括 GPU 推理费（A10G 约 $0.76/小时，每图约 15 秒，5 图约 $0.016）、Prompt 设计人力（约 2 小时/品类，$50/小时）、SAM 分割预处理（免费开源）。总成本约 **$100-150/品类**（含人工调优）。
- **合规**：Amazon 主图政策要求主图必须为纯白底（RGB 255,255,255），场景图仅允许用于副图（Alt Images）或 A+ 页面。若将场景图用作主图，违反 Amazon 图片政策，可能导致 Listing 下架。GDPR 无直接冲突，但场景中若出现人脸（如婴儿模型）需确保为 AI 生成而非真实人物。
- **风险**：次生风险包括（1）竞品通过反向图片搜索发现 AI 生成痕迹后，可能向 Amazon 投诉“图片与实物不符”；（2）场景图 CTR 过高但转化率下降（用户预期与实物差距大），导致退货率上升；（3）平台审查算法可能标记 AI 生成图片为“低质量内容”，影响 Listing 权重。
...
**三轨验证**：
- **成本**：显性成本为 GPU 批量推理费（80 SKU × 1 场景 × 15 秒 = 1200 秒 ≈ 0.33 小时，约 **$0.25**）+ Prompt 模板设计（1 小时，$50）+ 质量抽检人力（2 小时，$100）。总成本约 **$150**，远低于传统摄影 $40,000。
- **合规**：节日主题图需注意（1）Amazon 节日标签使用限制（如“Christmas”不可用于非宗教商品）；（2）场景中若包含圣诞老人、雪花等元素，需确保不侵犯迪士尼等 IP 版权；（3）GDPR 要求场景图中不可出现可识别个人身份的信息（如真实房屋外观）。
- **风险**：次生风险包括（1）批量生成导致图片风格高度雷同，Amazon 算法可能判定为“重复内容”或“自动化生成低质内容”；（2）节日后未及时切换回常规主图，导致淡季 CTR 骤降；（3）若场景图与商品实际使用场景不符（如婴儿玩具放在雪地中，暗示户外使用但产品仅限室内），可能触发消费者保护投诉。
