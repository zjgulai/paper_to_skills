...
**三轨验证**：
- **成本**：LLM API 调用费用约 ¥0.5-2/次（GPT-4o），周报场景每周 1 次 + 异常触发约 3-5 次/周，年化成本 ¥200-600；数据采集需对接 BI 系统 API，开发成本约 2-3 人天
- **合规**：输入数据需脱敏（去除订单号/客户邮箱等 PII），避免违反 GDPR/CCPA；输出建议不得包含操纵评论、刷单等违规内容；需在提示词中注入合规约束
- **风险**：LLM 幻觉可能导致错误归因（如将正常波动误判为竞品攻击），需人工复核关键决策；自动化建议若被盲目执行，可能引发跟价过度导致利润下滑；建议设置"人工确认"环节作为 P0 行动的前置条件
