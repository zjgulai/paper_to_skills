...
**三轨验证**：
- **成本**：
  - 数据采集：Google Trends API 免费（配额内），Helium10 订阅约 ¥800-1500/月，Amazon 广告报告免费
  - 计算资源：Kalman 滤波 + 加权融合每日运行约 0.5 小时（单机即可），年成本约 ¥2000-5000
  - 人力：数据接入 + 模型调优约需 1 人月（¥2-3 万）
  - **总显性成本：首年约 ¥4-5 万，后续每年约 ¥2-3 万**
- **合规**：
  - Amazon 政策：使用 Attribution 报告和广告报告属于卖家后台自有数据，合规；但不得使用爬虫抓取竞品搜索量（违反 ToS）
  - GDPR：若涉及欧洲站用户行为数据（如 Wishlist），需确保数据匿名化且不用于非必要目的
  - 广告法：不得基于感知到的需求进行虚假宣传或价格操纵
  - **合规状态：基本合规，但需避免爬虫行为**
- **风险**：
  - 信号误判：搜索趋势上升可能由短期营销活动（如网红推广）而非真实需求驱动，导致过度备货
  - 竞品反应：若竞品监测到搜索趋势同步备货，可能引发价格战
  - 平台审查：频繁基于广告信号调整库存可能被 Amazon 视为操纵排名（需谨慎）
  - **风险等级：中等，需设置信号置信度阈值和人工复核机制**
