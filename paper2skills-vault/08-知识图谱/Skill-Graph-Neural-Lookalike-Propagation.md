...
**三轨验证**：
- **成本**：显性成本约 2.5 万元（数据工程师 2 周工时 + 图计算服务器费用 0.5 万元），若使用现有 CRM 系统则边际成本可降至 0.8 万元
- **合规**：用户关系图基于第一方数据（注册信息 + 行为日志），不涉及第三方数据购买，符合 GDPR 和 CCPA 的合法利益基础；需在隐私政策中明确说明“基于用户行为关系进行相似受众扩展”
- **风险**：若种子用户本身存在偏差（如早期测试购买者非典型用户），图传播会放大偏差，导致扩展受众质量下降；建议每季度用 A/B 测试验证扩展受众的转化表现，及时调整种子筛选规则
...
**三轨验证**：
- **成本**：显性成本约 1.5 万元（TikTok API 数据采集开发 1 周 + 图计算 0.5 万元），若使用第三方社媒管理工具（如 Hootsuite）则需额外订阅费约 0.3 万元/月
- **合规**：TikTok 平台禁止未经授权的用户数据导出用于广告定向；需确保互动数据仅用于站内分析，不用于站外广告投放；建议通过 TikTok 官方“相似粉丝”功能或 Spark Ads 合作实现，避免违反平台服务条款
- **风险**：若粉丝互动行为被平台检测为“异常数据抓取”，可能导致账号限流或封禁；建议使用 TikTok 官方 API 的公开数据（如公开评论、公开收藏），不爬取私信或私密账号数据；同时准备备用方案（如直接投放 TikTok 官方 Lookalike）
