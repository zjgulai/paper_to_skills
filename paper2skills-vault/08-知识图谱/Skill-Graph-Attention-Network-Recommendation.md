...
**三轨验证**：
- **成本**：数据采集需购买历史订单数据（约 ¥0.5-2 万/月，若从ERP/平台API获取）；GPU训练成本约 ¥2000-5000/月（使用A100或同等算力）；人力成本约 1-2 名算法工程师 × 6-8 周（约 ¥8-15 万一次性投入）。
- **合规**：用户购买序列属于个人数据，需确保符合 GDPR（欧盟）/CCPA（加州）关于用户画像和自动化决策的知情同意要求；若用于 Amazon 站内推荐，需遵守 Amazon 广告政策中关于"基于购买历史推荐"的透明度要求；不可使用儿童数据（COPPA 合规）。
- **风险**：注意力权重可能放大"购买频率高但利润低"的商品推荐，导致客单价下降；若注意力机制过度关注近期购买，可能忽略长周期复购商品（如婴儿车），造成推荐品类窄化；模型可解释性虽优于黑盒模型，但仍需人工审核注意力权重合理性，避免出现"买吸奶器推荐避孕套"等尴尬推荐。
