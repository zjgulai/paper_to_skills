...
**三轨验证**：
- **成本**：Qdrant 自托管单节点（4核16G）月成本约 ¥500；人力投入约 2 人天（含索引调优）。
- **合规**：向量仅存储 Skill 文本嵌入，不涉及用户 PII 数据，无 GDPR/CCPA 风险；Qdrant 开源版无数据外泄风险。
- **风险**：HNSW 参数（ef_search）调低可能导致召回下降，影响下游 RAG 质量；需设置 recall 监控告警。
...
**三轨验证**：
- **成本**：CLIP 编码推理需 GPU（T4 即可），单张图片编码成本约 ¥0.003；每月 10 万新品入库约 ¥300 算力费。
- **合规**：产品图片需确保不包含儿童裸露/暴力内容，否则违反 Amazon 成人内容政策；建议部署 NSFW 过滤前置。
- **风险**：CLIP 对非英语/小语种产品图（如泰语包装）识别精度下降 10-15%，需准备人工标注兜底；图文融合可能引入视觉噪声，导致检索结果偏离用户意图。
