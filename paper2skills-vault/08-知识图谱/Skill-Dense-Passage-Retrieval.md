...
**三轨验证**：
- **成本**：显性成本包括：① 数据采集：需提取商品标题/要点/描述文本，约 2-3 人天；② 计算资源：BERT 模型推理需 GPU（如 T4 实例），离线编码 10 万商品约 4 小时，成本约 ¥200；③ 人力：算法工程师 2-3 周开发集成，约 ¥3-5 万。总启动成本约 ¥4-6 万。
- **合规**：① 独立站自建搜索，不涉及 Amazon 平台 API 违规；② 商品文本数据属于自有资产，无 GDPR 风险；③ 搜索结果仅影响排序，不涉及广告法虚假宣传。整体合规风险低。
- **风险**：① 若向量模型未针对母婴品类微调，可能出现语义漂移（如"quiet"误匹配"silent night"婴儿床）；② 纯向量召回可能丢失精确 SKU 匹配（如"P001"型号），需保留 BM25 作为兜底；③ 无次生竞品风险，但需监控搜索日志防止用户隐私泄露。
