...
**三轨验证**：
- **成本**：显性成本包括数据采集（需从广告平台 API 拉取 90 天关键词级数据，约 0.5 人天/次）、计算资源（单次训练 < 1 小时，云成本约 5-10 元）、人力（模型开发 2-3 人天，后续月度维护 0.5 人天）。若使用第三方数据清洗工具，额外增加 1000-3000 元/年。
- **合规**：需确保关键词数据不包含用户个人身份信息（PII），符合 GDPR 匿名化要求；Amazon 广告平台允许基于历史表现优化出价，但禁止使用种族、健康等敏感词作为特征；不涉及广告法虚假宣传红线。
- **风险**：次生风险包括：① 高预测 CVR 词可能引发竞品跟投，导致 CPC 上涨 10-20%，侵蚀 ROAS；② 平台审查：若模型过度依赖“cheap”“discount”等词，可能被 Amazon 判定为低价诱导，触发 listing 审核；③ 品牌损伤：对低 CVR 词大幅降 bid 可能导致品牌词曝光不足，被竞品抢占。
...
**三轨验证**：
- **成本**：显性成本包括词义相似度计算（需维护 TF-IDF 词向量库，初始构建 1-2 人天，后续增量更新 0.2 人天/月）、冷启动推理（单次 < 1 秒，无额外计算成本）、人工审核（建议每周 0.5 人天复核预测结果）。若使用外部词向量 API（如 Google Word2Vec），年费约 500-2000 元。
- **合规**：冷启动依赖的历史词 CVR 数据需来自同一品类，避免跨品类数据污染；词义相似度计算不涉及用户隐私，符合 GDPR；Amazon 政策允许基于语义相似性推荐关键词，但禁止直接复制竞品品牌词。
- **风险**：次生风险包括：① 词义相似度偏差：TF-IDF 可能将“baby carrier”与“baby bottle”误判为高相似，导致冷启动 CVR 预测偏高，测试期浪费预算；② 平台审查：若冷启动推荐 bid 过高，可能触发 Amazon 的异常出价检测，导致账户被限流；③ 品牌损伤：对无历史数据的长尾词盲目投放，可能因 landing page 不匹配导致差评率上升。
