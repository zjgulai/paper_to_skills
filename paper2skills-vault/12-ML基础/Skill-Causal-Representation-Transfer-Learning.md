...
**三轨验证**：
- **成本**：数据采集成本约$2万（需购买日本站3个月用户行为日志，含API调用费）；计算资源成本约$0.5万（GPU训练+存储）；人力成本约$5万（1名ML工程师+1名业务分析师，2个月）。总显性成本约$7.5万。
- **合规**：用户行为数据跨域使用需确保符合GDPR（欧盟用户）和CCPA（加州用户）规定；日本站数据需遵守《个人信息保护法》（APPI），需获得用户明确同意用于模型训练；不触碰Amazon政策红线（仅使用站内行为数据，不爬取竞品数据）。
- **风险**：若CORAL对齐过度，可能消除域间合理差异（如日本用户对品牌信任度的真实高权重），导致模型在日本站过度泛化；若不变特征选择错误（如将"价格敏感度"误判为不变特征），可能引发定价策略失误，导致转化率下降5-10%；建议保留20%域特异特征作为安全缓冲。
...
**三轨验证**：
- **成本**：数据清洗与特征工程约$3万（需对齐两个类目的产品属性与用户行为标签）；模型训练与验证约$1万（使用现有GPU集群）；人力成本约$4万（1名数据科学家+1名品类经理，1.5个月）。总显性成本约$8万。
- **合规**：跨品类用户数据使用需符合平台数据使用协议（如Amazon的"数据不得用于非授权品类"条款）；若用户未授权跨品类分析，可能违反GDPR的"目的限制原则"；建议在用户协议中增加"品类扩展分析"条款，或使用匿名化聚合数据。
- **风险**：辅食与护肤品的用户决策路径差异较大（辅食为刚需高频，护肤为可选低频），若IRM错误地将"促销响应"识别为不变特征，可能导致护肤品类过度促销，毛利率下降3-5%；次生风险包括：若模型迁移失败导致库存预测偏差，可能引发护肤品类缺货（损失$5-10万销售额）或滞销（库存成本增加$2-4万）；建议在迁移初期设置20%的预测置信度阈值，低于阈值时回退到规则模型。
