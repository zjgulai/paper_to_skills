...
> **论文**：Nested versus non-nested cross-validation for machine learning | **arXiv**：1809.09446
...
- **成本轨**：
  - 数据采集：已有历史销量数据，无额外采集成本（0 元）
  - 计算资源：Prophet 模型 × 3 折 × 30 SKU，单次运行 ~2 小时 GPU 时间，月度运行成本 ~500 元（云服务）
  - 人力投入：数据工程师 1 人 × 3 天（数据清洗+验证脚本开发），成本 ~3000 元
  - **总成本**：~3500 元/月

- **合规轨**：
  - ✓ 数据合规：仅使用内部销量数据，无涉及用户隐私信息，符合 GDPR
  - ✓ 平台政策：Amazon 销售数据分析属于常规运营，无政策冲突
  - ✓ 跨境贸易：销量预测用于库存规划，属于合法商业决策，无贸易法规风险
  - **合规结论**：完全合规

- **风险轨**：
  - 竞品风险（概率 5%）：若预测精度提升导致库存优化明显，竞品可能跟进采用相同方法，但不构成竞争威胁
  - 平台审查风险（概率 <1%）：销量预测属于常规分析，无触发审查的风险
  - 模型失效风险（概率 15%）：时序模式突变（如疫情、营销活动）导致预测失效，需配置预警机制
  - **主要风险**：模型失效导致库存决策失误，建议设置 MAPE > 25% 的自动告警
...
- **成本轨**：
  - 数据采集：从用户行为日志中提取会话数据，已有基础设施，无额外采集成本（0 元）
  - 计算资源：LightGBM 模型 × 5 折 × 超参网格搜索（12 组参数），总计 ~8 小时 CPU 时间，成本 ~800 元
  - 人力投入：数据科学家 1 人 × 5 天（特征工程+模型开发+验证），成本 ~5000 元
  - **总成本**：~5800 元/周期（模型迭代周期 4 周）

- **合规轨**：
  - ⚠️ 数据合规：会话数据涉及用户行为追踪，需确保：
    - 用户已同意行为数据采集（检查隐私政策）
    - 数据已脱敏处理（user_id 加密）
    - 符合 GDPR 数据最小化原则（仅保留必要特征）
  - ✓ 平台政策：Amazon 允许基于用户行为的个性化推荐，无政策冲突
  - ✓ 广告法：预测用于内部转化优化，不涉及虚假宣传
  - **合规结论**：条件合规（需确认用户同意和数据脱敏）

- **风险轨**：
  - 隐私风险（概率 8%）：若 user_id 映射表泄露，可能反推用户身份，建议加强数据访问控制
  - 模型偏差风险（概率 12%）：GroupCV 可能因用户分布不均（活跃用户 vs 僵尸用户）导致评估偏差，需分层分析
  - 营销效果风险（概率 20%）：即使 AUC 正确，模型预测的"高价值新客"可能因营销成本高而不经济，需配合 ROI 分析
  - **主要风险**：模型上线后转化率不达预期，建议灰度发布（10% 流量）+ 周度监控
