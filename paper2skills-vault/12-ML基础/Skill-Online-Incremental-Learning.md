...
**三轨验证**：
- **成本**：需搭建流式数据管道（Kafka/Flink 或 AWS Kinesis），初期工程投入约 ¥5-8 万；River 库免费，但需 0.5 个 ML 工程师月维护
- **合规**：不触碰 Amazon 政策红线；仅使用公开销量与促销日历数据，无用户隐私风险
- **风险**：模型过度敏感可能导致补货决策频繁波动，需设置更新阈值（如漂移置信度 > 95% 才触发补货调整）
...
**三轨验证**：
- **成本**：广告点击流数据已由平台提供（Amazon Ads API），无额外采集成本；FTRL 模型计算量极低，单台服务器可支撑百万级 QPS
- **合规**：需确保出价策略不违反 Amazon 广告政策（如不得使用"点击欺诈"或"恶意竞价"）；FTRL 仅调整出价幅度，不操纵排名
- **风险**：快速响应可能引发竞品价格战（若竞品也使用在线学习），建议设置出价上限（如不超过 ROAS 目标 1.5 倍）；平台可能审查异常出价模式，需保留日志备查
