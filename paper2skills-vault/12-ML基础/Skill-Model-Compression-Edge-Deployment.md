...
**三轨验证**：
- **成本**：PDA 设备采购/升级费用约 3-5 万元（若现有设备不支持 ONNX runtime）；模型量化验证需 2 人天工程师时间（约 0.6 万元）
- **合规**：本地推理不传输用户数据，无 GDPR 跨境风险；需确保 PDA 系统不记录无关员工行为数据
- **风险**：模型精度漂移导致推荐错误 SKU，可能引发仓库发货错误；建议部署后前 3 个月人工复核推荐结果
...
**三轨验证**：
- **成本**：Serverless 函数部署费用约 0.5 万元/年（含流量）；模型转换与测试需 3 人天（约 0.9 万元）
- **合规**：定价模型需确保不违反 Amazon 平台「最低价政策」及出海目标国「反价格歧视法」；需保留定价日志备查
- **风险**：边缘模型与云端模型定价不一致可能导致价格战或用户投诉；建议部署 A/B 对比机制，边缘推理结果偏差超过 5% 时自动回退云端
