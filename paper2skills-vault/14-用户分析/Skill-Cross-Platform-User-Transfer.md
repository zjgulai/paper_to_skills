...
**三轨验证**：
- **成本**：Amazon 订单导出 API 调用费用（约 $0.01/次 × 500 用户 = $5/周）；偏好提取与域适配模型训练需 1 名算法工程师 2 周（人力成本约 ¥2 万）；独立站服务器额外计算资源可忽略。
- **合规**：Amazon 订单导出需用户明确授权（通过独立站注册时的勾选同意）；GDPR 下需提供数据删除选项；不得将 Amazon 原始订单数据存储超过 30 天（仅保留聚合偏好向量）。
- **风险**：若 Amazon 检测到批量导出行为可能触发 API 限流或账号审查；用户对跨平台数据共享的知情权不足可能导致品牌信任损伤；迁移模型偏差（Amazon 促销偏好被误迁移为品牌偏好）导致推荐不准。
