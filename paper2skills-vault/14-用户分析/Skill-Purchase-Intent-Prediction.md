...
**三轨验证**：
- **成本**：需部署用户行为埋点（GA4/自建SDK），月均数据存储与计算成本约 ¥2,000-5,000；邮件自动化工具（如Klaviyo）月费约 $150-500；人力投入约 2 周开发+1 周测试。
- **合规**：需确保邮件退订链接清晰可见（CAN-SPAM法案）；GDPR下需用户同意行为追踪；独立站不受Amazon政策约束，但需遵守当地消费者保护法。
- **风险**：高意图用户误判（如比价行为被误判为购买意图）可能导致过度优惠，降低利润；低意图用户长期不触达可能流失至竞品；邮件频率过高可能触发ISP封禁域名。
...
**三轨验证**：
- **成本**：Amazon DSP 最低月消耗 $10,000+；Attribution 数据订阅费约 $500/月；需数据分析师 0.5 FTE 维护模型与出价策略；计算资源可复用现有云环境。
- **合规**：Amazon DSP 禁止使用非Amazon收集的第三方数据用于受众定向；意图评分不得基于种族、健康、政治等敏感信息；出价策略需符合Amazon广告政策（禁止歧视性定价）。
- **风险**：高意图用户出价过高可能引发竞品跟价，推高整体CPC；Amazon可能因算法歧视审查出价逻辑；若意图模型准确率下降，ROAS可能骤降；品牌方可能因过度投放产生广告疲劳。
