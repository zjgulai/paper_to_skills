...
**三轨验证**：
- **成本**：需接入实时事件流（如 Kafka/PubSub），数据工程师 2 周 + 模型开发 2 周；GPU 训练成本约 ¥500/月（轻量 SASRec 无需大规模预训练）
- **合规**：行为序列本身不涉及 PII（仅 event_type + item_id），符合 GDPR 匿名化要求；但需注意 Amazon 政策禁止基于"购买意图"进行歧视性定价（如对高意图用户提价）
- **风险**：若对"比较决策"用户频繁推送优惠，可能引发用户对价格敏感度上升（等待优惠才下单），长期侵蚀品牌溢价能力；同时可能被竞品监测到定价策略
...
**三轨验证**：
- **成本**：数据采集需埋点升级（约 1 周开发）；GPU 训练成本 ¥500-2000/月（取决于序列长度和用户量）；人力成本约 ¥8 万（3-4 周算法工程师）
- **合规**：序列数据若包含 IP/Device ID 可能触发 GDPR 个人数据定义；Amazon 政策禁止基于行为序列进行"价格歧视"（如对高意图用户隐藏低价选项）；建议使用差分隐私或聚合统计
- **风险**：过度依赖序列模型可能导致"信息茧房"（用户只看到相似品类）；若模型误判"比较决策"（实际是帮朋友看），推送优惠可能适得其反；长期可能被平台视为操纵用户行为（Amazon 反操纵政策）
