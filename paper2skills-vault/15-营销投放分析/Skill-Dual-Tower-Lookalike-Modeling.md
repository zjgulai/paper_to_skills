...
**三轨验证**：
- **成本**：数据管道搭建（ETL + 特征工程）约 8 万元，模型训练 GPU 资源约 2 万元/月，人力成本（1 名数据工程师 + 0.5 名分析师）约 5 万元/月，首月总投入约 15 万元，后续每月固定成本约 7 万元。
- **合规**：用户数据 hash 后上传 Meta Custom Audience 符合 Meta 数据使用政策；需确保用户已同意数据用于广告优化（GDPR 下需明确 opt-in）；Amazon 独立站场景下不涉及平台政策冲突。
- **风险**：自建 Lookalike 受众质量若低于平台内置版本，可能导致 ROAS 不升反降；Meta 可能因数据匹配率低（<60%）降低受众投放权重；过度依赖单一受众包可能导致广告疲劳，需配合频次控制。
...
**三轨验证**：
- **成本**：跨品类行为映射模型开发约 5 万元，DSP 数据采购（合规第三方数据）约 3 万元/品类，TikTok Shop 广告测试预算约 2 万元/品类，总成本约 10 万元/品类。
- **合规**：需确保竞品用户数据通过合法 DSP 渠道获取（如 Amazon Marketing Cloud 或第三方数据提供商），不得直接爬取或购买用户隐私数据；TikTok Shop 广告政策禁止使用未经授权的用户画像数据。
- **风险**：跨品类映射假设（哺乳期→推车期）可能因用户行为偏差失效（如部分用户跳过推车直接购买玩具）；冷启动期若受众质量不足，可能导致 TikTok 账户质量分下降，影响后续投放；品牌损伤风险：若推车品类广告频繁触达非目标用户，可能引发用户反感。
