...
**三轨验证**：
- **成本**：显性成本包括 Amazon 广告 API 调用费（约 $0.003/次，每日约 500 次调用）、数据存储（AWS S3 约 ¥200/月）、人力投入（1 名数据工程师 2 周开发 + 1 名运营每周 2 小时调参）。首年总成本约 ¥3-5 万。
- **合规**：Amazon 广告 API 允许基于 ROAS/曝光/新客的自动出价，但禁止使用竞品品牌词作为目标；GDPR 要求新客识别数据（像素/Attribution）需用户明确同意；广告法禁止虚假宣传"新客占比"数据。需确保数据脱敏和用户授权。
- **风险**：旺季激进出价可能引发竞品跟涨，导致 CPC 整体抬升 10-20%；平台可能因频繁调价触发"异常活动"审查；若新客占比目标设置过高，可能牺牲老客复购，造成品牌忠诚度下降。
