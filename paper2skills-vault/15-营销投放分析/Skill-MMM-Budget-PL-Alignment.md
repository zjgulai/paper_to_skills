...
**三轨验证**：
- **成本**：需采购第三方 MMM 工具（如 Recast/Kaliber）或自建模型，月均成本 $500-1,500；数据清洗与毛利率映射需 1 名分析师 2 周。
- **合规**：Amazon PPC 预算削减需注意不影响品牌搜索排名（建议保留最低投放 $3K）；TikTok 投放需遵守 FTC 网红营销披露规则。
- **风险**：过度依赖 TikTok 可能导致渠道集中度风险（算法变更或政策封禁）；配件低毛利渠道削减后可能影响用户复购（配件是高频复购品）。
...
**三轨验证**：
- **成本**：Hindsight Regret 分析仅需历史数据回算，计算成本极低（< $50/次）；但需确保数据颗粒度到日级别，否则拟合偏差大。
- **合规**：使用历史 GMV 数据不涉及用户隐私，GDPR 合规；但若使用 Amazon 归因窗口（14 天点击）需确认数据授权范围。
- **风险**：Hindsight Regret 是后验分析，直接用于下轮预算分配可能过拟合（需结合贝叶斯先验平滑）；过度削减 Amazon PPC 可能触发平台广告账户质量分下降。
