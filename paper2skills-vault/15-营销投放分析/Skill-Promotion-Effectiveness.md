...
- **前置**：A/B Experimental Design（促销效果评估需要对照组设计）
- **前置**：Uplift Modeling（CATE估计是核心工具）
- **延伸**：Customer Lifetime Value（促销带来的增量用户，其长期LTV才是完整ROI）
- **可组合**：+ Marketing Mix Modeling → MMM看渠道级效果，促销分析看用户级效果
