...
**三轨验证**：
- **成本**：需接入用户行为日志（浏览/点击/加购）和订单数据，数据清洗与特征工程约2人周；DML模型训练需GPU资源（单次约$50-100云成本）
- **合规**：需确保用户分群不基于敏感属性（种族/宗教/健康）；定向发券需符合Amazon Coupon政策（不得歧视性定价）；GDPR下需用户同意行为追踪
- **风险**：若自然流量用户发现被取消折扣，可能引发客诉与品牌信任下降；广告流量用户增量率高但客单价低，过度依赖可能拉低品牌定位
...
**三轨验证**：
- **成本**：需搭建A/B测试框架（至少3个实验组+1个对照组），样本量需满足统计功效（每组至少2000用户）；分析周期约2周
- **合规**：黑五期间Amazon要求促销价格不得高于过去30天最低价（参考价政策）；折扣幅度超过30%可能触发平台价格审计
- **风险**：30% off可能引发竞品跟进价格战，导致品类整体利润下滑；高折扣可能被消费者视为"品牌廉价化"，影响长期品牌溢价；若增量订单主要来自时间转移（提前消费），黑五后1-2月销售额可能骤降
