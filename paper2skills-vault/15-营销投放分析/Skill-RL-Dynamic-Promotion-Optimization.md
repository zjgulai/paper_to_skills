...
**三轨验证**：
- **成本**：需采购或自建 RL 推理服务（约 ¥3-8 万/年）；历史 A/B 实验数据清洗与特征工程约 2 人周；线上 A/B 测试流量分流需占用 10-20% 用户池。
- **合规**：Amazon 政策禁止基于敏感属性（种族/健康等）的差异化定价；GDPR 要求用户可拒绝个性化促销（需提供 opt-out 接口）；欧盟《数字服务法案》要求高透明度——需向用户解释"为何收到此折扣"。
- **风险**：若 RL 策略过度集中于高意图用户，可能导致低意图用户长期被忽略而流失；折扣力度过大可能触发平台比价机制或竞品跟进价格战；模型冷启动阶段若探索策略不当，可能发送过多无效促销引发用户投诉。
...
**三轨验证**：
- **成本**：线上推理需部署模型服务（GPU/CPU 实例约 ¥5-15 万/年）；离线训练需每周全量数据更新（Spark 集群约 ¥2-4 万/月）；人力成本：1 名算法工程师 + 0.5 名数据工程师（约 ¥50-80 万/年）。
- **合规**：Amazon 政策禁止基于地理位置/设备类型等非行为特征的差异化定价；GDPR 第 22 条要求自动化决策需提供人工复核通道；中国《个人信息保护法》要求促销算法需通过个人信息保护影响评估（PIA）。
- **风险**：RL 可能学到"对已流失用户发送最大折扣"的次优策略，导致利润率持续下降；若竞品同步采用 RL 促销，可能陷入折扣螺旋（双方不断加大力度）；模型黑箱特性在监管审查时难以解释单个促销决策的合理性。