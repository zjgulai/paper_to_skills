...
**三轨验证**：
- **成本**：Amazon Product Advertising API 调用费约 $0.01/次，每日扫描 5000 ASIN 约 $50/天；图像哈希计算需 1 台轻量 GPU 实例（约 ¥300/月）；人力成本主要为初始规则配置（2 人天）和每周审核预警（0.5 人天/周）。
- **合规**：Amazon 品牌备案投诉需确保证据真实，虚假举报可能导致品牌备案被撤销；DMCA 举报需提供版权登记证明，否则可能被驳回；GDPR 不直接适用（监控对象为商品而非个人数据），但需注意不抓取卖家个人信息。
- **风险**：误报率约 5-10%，错误举报可能引发竞品反诉或平台审查；频繁举报可能导致 Amazon 对品牌账号的信任度下降；若被竞品发现监控手段，可能转向更隐蔽的侵权方式（如站外引流）。
