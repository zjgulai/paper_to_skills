...
**三轨验证**：
- **成本**：显性成本包括：① 数据采集：需接入支付网关（Stripe/PayPal）API 获取交易、设备指纹、IP 数据，约 $200-500/月（API 调用费+第三方设备指纹服务）；② 计算资源：实时推理需部署轻量级模型（如 ONNX），单台 4 核 8G 云服务器（约 ¥500/月）可支撑 1000 QPS；③ 人力：规则+图特征版需 1 名算法工程师 2-3 周（约 ¥2-3 万），GNN 深度版需 6-8 周（约 ¥6-8 万）。
- **合规**：① 设备指纹采集需在隐私政策中明确告知用户（GDPR 第 13 条），欧盟用户需 opt-in 同意；② 信用卡号仅存储后 4 位（PCI-DSS 合规），全号不可落盘；③ 欺诈评分不得用于拒绝服务后的歧视性定价（如对高风险用户加价），可能违反《欧盟数字服务法》或美国《平等信用机会法》；④ 独立站场景不涉及 Amazon 政策，但若使用 Shopify Payments 需遵守其 AUP（可接受使用政策）。
- **风险**：① 误杀风险：若模型误判正常订单为欺诈（假阳性率 > 5%），将导致客户流失和客诉（高客单价母婴用户复购率关键）；② 对抗攻击：欺诈团伙可能通过模拟正常行为（如延长会话时间、使用住宅 IP）绕过规则，需持续更新特征；③ 品牌损伤：若自动拒绝订单后未提供友好解释（如“请联系客服验证”），可能引发社交媒体负面评价；④ 数据泄露：设备指纹和 IP 数据若被内部滥用，可能违反数据保护法并导致罚款（GDPR 最高 2000 万欧元或全球年营收 4%）。
