...
**三轨验证**：
- **成本**：显性成本包括：① 基础模型推理费用（Chronos 模型单次推理约 ¥0.002/时序点，280 个指标/天 → ¥0.56/天）；② 数据管道维护（云函数 + 存储约 ¥500/月）；③ 运营人员培训（约 2 小时）。总年化成本约 ¥1.2 万。
- **合规**：① 销量/退货率等聚合指标不涉及用户隐私，无 GDPR 风险；② 异常检测结果不直接用于广告投放决策，不触碰 Amazon 广告政策红线；③ 若异常检测触发自动调价/自动下架，需人工复核，否则违反平台自动决策合规要求。
- **风险**：① 误报风险：正常促销波动被标记为异常，导致运营误判（如大促前销量自然增长被标记为刷单）；② 漏报风险：模型对缓慢漂移型异常（如退货率逐月上升）不敏感；③ 品牌损伤：若基于异常检测自动暂停广告，可能错失转化窗口。
