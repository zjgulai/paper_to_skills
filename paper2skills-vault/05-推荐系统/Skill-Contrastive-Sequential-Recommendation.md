...
**三轨验证**：
- **成本**：显性成本包括 GPU 训练资源（约 ¥3,000-5,000/月，若使用 AWS p3.2xlarge 实例）、数据采集与清洗人力（约 2 周，¥8,000-12,000）、模型部署与维护（约 1 周，¥4,000-6,000）。总首次投入约 ¥15,000-23,000。
- **合规**：自监督训练仅使用用户行为序列，不涉及 GDPR 敏感数据（如姓名、地址、支付信息），合规风险低。但需确保用户行为数据采集已获得用户同意（Cookie 政策/隐私协议），且不用于跨站追踪（避免触碰 Amazon 广告政策红线）。
- **风险**：次生风险包括：① 若负例采样不当（如将同一用户的不同会话误判为负例），可能导致推荐同质化，降低用户满意度；② 模型冷启动初期（前 2 周）推荐质量可能低于随机推荐，需设置兜底策略（如热门推荐）；③ 若独立站流量持续低迷（月 UV < 2,000），对比学习增益有限，ROI 可能为负。
