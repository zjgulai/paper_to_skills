...
**三轨验证**：
- **成本**：数据采集（埋点+历史订单清洗）约 ¥2 万；LambdaMART 模型训练与部署约 3 人周；线上 A/B 测试服务器资源约 ¥0.5 万/月。首月总成本约 ¥4-5 万。
- **合规**：用户历史数据需脱敏处理，避免存储明文 user_id 与购买记录的关联；GDPR 下需用户同意行为追踪；Amazon 站内搜索个性化需遵守平台排名公平性政策，不得人为压低非付费商品。
- **风险**：过度个性化可能导致信息茧房，用户看不到新品或跨品类商品；老用户历史偏好权重过高会抑制探索性购买；若模型偏差导致某些品类始终低排名，可能引发供应商投诉。
