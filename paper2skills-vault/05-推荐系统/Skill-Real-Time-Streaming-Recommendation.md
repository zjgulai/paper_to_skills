...
**三轨验证**：
- **成本**：显性成本包括 Kafka 集群（约 ¥3-5 万/年）、Redis 实例（约 ¥1-2 万/年）、FAISS 向量检索服务器（约 ¥2-4 万/年）、开发人力（约 6-8 周，折合 ¥4-6 万）。总首年成本约 ¥10-17 万。
- **合规**：独立站场景下不涉及 Amazon 政策；需确保 GDPR 合规——用户行为数据采集需有明确 consent，且支持用户随时删除个人数据（Redis 中需实现 TTL 或手动清除机制）；不涉及广告法红线。
- **风险**：次生风险包括：① 实时推荐可能导致用户隐私感知过强（“被监视感”），需在 UI 上提供推荐解释或关闭选项；② 若推荐更新过于频繁（每次点击都刷新），可能造成用户困惑或体验下降，需设置合理的更新节流（如 3 秒内不重复刷新）；③ 独立站流量较小时，实时推荐效果可能不如批量系统稳定，需设置 fallback 策略。
