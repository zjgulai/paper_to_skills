...
**三轨验证**：
- **成本**：显性成本约 ¥8-15 万/年（数据采集系统改造 ¥3-5 万，DoubleML 模型训练与部署 ¥3-6 万，因果图维护人力 ¥2-4 万）。需额外投入因果推断专家 0.5 人天/周。
- **合规**：① 价格弹性估计基于历史交易数据，不涉及用户画像，GDPR 风险低；② 若使用 A/B 实验数据验证，需确保实验组价格差异不超过 Amazon 价格公平政策阈值（通常 ±15%）；③ 广告花费数据需脱敏，避免暴露营销策略。
- **风险**：① 因果图误设风险：若遗漏关键混淆变量（如竞品价格），因果效应估计仍可能偏差；② 大促期极端分布偏移可能导致因果 Bandit 探索不足，错过最优价格；③ 若因果 RL 策略频繁调价，可能触发 Amazon 价格监控算法，导致 listing 被标记。
