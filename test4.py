...
# Since regex sub and normal replace failed due to different white space or minification issues,
# Let's replace the whole `build_chat_page_js` block accurately.

start_js_idx = content.find('def build_chat_page_js() -> str:\n    return r"""')
if start_js_idx != -1:
    end_js_idx = content.find('"""\n\n\ndef build_search_js() -> str:', start_js_idx)
