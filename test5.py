...
# I need to modify the RAG_JS inside `_post_build_patch`
# Wait, I previously modified `build_chat_page_js` but it seems there's another hardcoded override in `_post_build_patch`!
# Let's replace the whole RAG_JS definition in `_post_build_patch`

old_str_start = '        RAG_JS = r"""(function () {\n'
old_str_end = '})();\n"""\n        chat_js_path.write_text(RAG_JS, encoding="utf-8")\n'

start_idx = content.find(old_str_start)
end_idx = content.find(old_str_end, start_idx) + len(old_str_end)

if start_idx != -1 and end_idx != -1:
    new_js = r'''        RAG_JS = r"""(function () {
...
'''
    ...
    with open('paper2skills-skills/playbook-generator/scripts/build_playbook.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Replaced RAG_JS correctly in _post_build_patch.")
else:
    print("Failed to find RAG_JS in _post_build_patch")
