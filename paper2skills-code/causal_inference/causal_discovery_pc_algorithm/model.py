"""
Skill: Skill-Causal-Discovery-PC-Algorithm
...
def generate_causal_data(n=1000, seed=42):
    import numpy as np, pandas as pd
    np.random.seed(seed)
    X = np.random.randn(n, 3)
    treatment = (X[:, 0] + np.random.randn(n) > 0).astype(float)
    outcome = 2 * treatment + X[:, 0] * 0.5 + np.random.randn(n)
    ...
    try:
        ...
        for k, v in result.items():
            if isinstance(v, dict):
                ...
                print(f"  {k}: {v}")
        print(f"[OK] Causal-Discovery-PC-Algorithm 测试通过")
