"""
Skill: Skill-Probabilistic-Hierarchical-New-Product-Forecast
Domain: growth_model
Title: Probabilistic Hierarchical New Product Forecast（概率层次新品销量预测）
"""
import numpy as np
import pandas as pd
import warnings
...
def generate_customer_data(n=2000, seed=42):
    import numpy as np, pandas as pd
    np.random.seed(seed)
    recency = np.random.randint(1, 365, n)
    frequency = np.random.randint(1, 50, n)
    monetary = frequency * np.random.uniform(20, 100, n)
    churn = ((recency > 180) & (frequency < 3)).astype(int)
    return pd.DataFrame({"recency": recency, "frequency": frequency,
                         "monetary": monetary, "churn": churn,
                         "customer_age": np.random.randint(1, 1000, n)})
    ...
    try:
        ...
        for k, v in result.items():
            if isinstance(v, dict):
                ...
                print(f"  {k}: {v}")
        print(f"[OK] Probabilistic-Hierarchical-New-Product-Forecast 测试通过")
