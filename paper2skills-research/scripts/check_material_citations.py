...
def main() -> int:
    ...
    if bad:
        print("\n⇒ exit 1：材料引文有问题。处置由人定（换材料真词 / 改标「业务侧默认＋替换条件」/"
              "改成逐字原文）。")
        return 1
    # ⚠️ **这句话必须带上适用范围**（2026-09-13，主控）。
    #    此前它只说「全部材料引文有据」，而本判据的**扫描单元是「行」**且要 `材料` 与引号同行 ——
    #    实测同一批 139 份契约里仍有 `「季度经营策略」` 40 份、`材料 §F.3`/`§E.2` 一族 23 份，
    #    全都出现在 **blockquote 续行**上，本判据一条都看不见（台账 #67）。
    #    **一条测不出来的政策不是政策**；绿可以，但绿不许读成「一处不剩」。
    print("\n⇒ exit 0：**本判据适用范围内**（扫描单元＝行；`材料` 与引号须同行同分句）全部引文有据。")
    print("   ⚠️ 这不等于「没有残留」：块级/续行形态的同类残留不在此判据视野内，"
          "另见 `scripts/check_material_residue.py`（台账 #67）。")
    return 0
