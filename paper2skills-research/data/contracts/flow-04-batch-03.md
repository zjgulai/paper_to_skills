...
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/A/CTR-A-010-趋势监测.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 A 类（⚠️ 来源＝本项目《经营侧组织模型精确抽取》`reports/_survey_org_model.md` §F.3 的判定，**二手来源，不是材料原文**，别改判）**：趋势、突变与季节性检测有成熟的时序分解与在线变点方法（BSTS、CUSUM）。
...
- 只有 legacy 预览版（可引 slug，但 §1 必须写明「论文实验参数未随卡进入本包」，清单按卡面可见数字列）：共 14 张
    ...
    - `p2s-blue-ocean-category-discovery` · 源卡号 `Skill-Blue-Ocean-Category-Discovery` · 06-增长模型 · Blue Ocean Category Discovery — AI 驱动的蓝海品类机会识别与先验测试
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-blue-ocean-category-discovery/SKILL.md`
    - `p2s-category-trend-forecasting` · 源卡号 `Skill-Category-Trend-Forecasting` · 06-增长模型 · Category Trend Forecasting（品类趋势预测）
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-category-trend-forecasting/SKILL.md`
    ...
    - `p2s-multimodal-new-product-sales-forecast` · 源卡号 `Skill-Multimodal-New-Product-Sales-Forecast` · 06-增长模型 · 多模态外部信号新品销量预测 — Google Trends + 图片融合预测
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-multimodal-new-product-sales-forecast/SKILL.md`
    - `p2s-sir-viral-product-adoption-forecasting` · 源卡号 `Skill-SIR-Viral-Product-Adoption-Forecasting` · 06-增长模型 · SIR爆品传播预测 — 流行病学模型迁移至TikTok短视频爆款需求预测
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-sir-viral-product-adoption-forecasting/SKILL.md`
    ...
    - `p2s-tiktok-trending-product-signal` · 源卡号 `Skill-TikTok-Trending-Product-Signal` · 15-营销投放分析 · TikTok爆款商品信号挖掘 — 话题标签增速分析提前发现下一个爆款品类
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-tiktok-trending-product-signal/SKILL.md`
...
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/A/CTR-A-064-知识产权检索.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 A 类（⚠️ 来源＝本项目《经营侧组织模型精确抽取》`reports/_survey_org_model.md` §F.3 的判定，**二手来源，不是材料原文**，别改判）**：专利/商标检索与相似度、侵权风险打分是标准IR与文本相似建模。
> 引用纪律：这句判定可以进正文，但**不得标成「材料 §F.3 原文」**。S1 实测 139 份作业包全都
> 把它标成了「材料 §F.3 原文」，撰写人于是写出「材料 §F.3 原文写明『流程合理性须业务确认』」
> 这类**把我们的分析记到材料账上**的句子 —— 核验器实测该句在材料里 0 命中。
...
- 只有 legacy 预览版（可引 slug，但 §1 必须写明「论文实验参数未随卡进入本包」，清单按卡面可见数字列）：共 8 张
    ...
    - `p2s-counterfeit-product-visual-detection` · 源卡号 `Skill-Counterfeit-Product-Visual-Detection` · 19-风控反欺诈 · 假冒商品视觉检测 — 感知哈希批量比对竞品图识别疑似仿品
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-counterfeit-product-visual-detection/SKILL.md`
    ...
    - `p2s-patent-prior-art-risk-scan` · 源卡号 `Skill-Patent-Prior-Art-Risk-Scan` · 21-合规决策 · 专利先验技术风险扫描 — 新品上市前侵权风险评估
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-patent-prior-art-risk-scan/SKILL.md`
    - `p2s-supply-chain-counterfeit-detection` · 源卡号 `Skill-Supply-Chain-Counterfeit-Detection` · 19-风控反欺诈 · Supply Chain Counterfeit Detection — 供应链仿冒品检测原材料/包装视觉验真
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-supply-chain-counterfeit-detection/SKILL.md`
- 仅精选线（尚未换底进产品线，引 `card_id`，并注明「S5 换底后替换为已装 slug」）：共 0 张
    （无）
...
| 域 / 面 | `DOM-02` / `PLN-OPS` |
| flows | FLOW-02, FLOW-04 |
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/B/CTR-B-014-产品范围管理.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 B 类（⚠️ 来源＝本项目《经营侧组织模型精确抽取》`reports/_survey_org_model.md` §F.3 的判定，**二手来源，不是材料原文**，别改判）**：范围变更影响可估算，但裁剪与承诺变更属立项治理决定。
> 引用纪律：这句判定可以进正文，但**不得标成「材料 §F.3 原文」**。S1 实测 139 份作业包全都
> 把它标成了「材料 §F.3 原文」，撰写人于是写出「材料 §F.3 原文写明『流程合理性须业务确认』」
> 这类**把我们的分析记到材料账上**的句子 —— 核验器实测该句在材料里 0 命中。
