...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：能力-需求匹配可建成向量检索/推荐排序问题，直接输出Skill与岗位装配的候选集。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：补货策略可用(s,S)/报童/多级库存仿真直接求解并输出服务水平-成本前沿。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：价格/广告/库存联合动作是带约束的多变量优化与预算分配问题。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：平台运营指标可建模并优化（流量、转化、库存联动），属可量化运营。
...
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/A/CTR-A-046-术语治理.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：术语抽取、跨语对齐与一致性校验是标准术语学+文本算法。
- **§F.5 边界条目**：术语抽取/对齐/一致性校验是标准文本算法 ⇒ 降级条件：若术语裁决必须由母语专家逐条确认，则降 B
...
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/A/CTR-A-057-增量分析.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：增量估计是因果推断核心问题（DID、合成控制、因果森林、地理实验）。
...
- 只有 legacy 预览版（可引 slug，但 §1 必须写明「论文实验参数未随卡进入本包」，清单按卡面可见数字列）：共 51 张
    ...
    - `p2s-causal-cohort-analysis` · 源卡号 `Skill-Causal-Cohort-Analysis` · 01-因果推断 · Causal Cohort Analysis — 因果队列分析：促销干预的长期用户行为效应
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-causal-cohort-analysis/SKILL.md`
    ...
    - `p2s-clustersc-synthetic-control` · 源卡号 `Skill-ClusterSC-Synthetic-Control` · 01-因果推断 · ClusterSC - 聚类合成控制法
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-clustersc-synthetic-control/SKILL.md`
    ...
    - `p2s-cross-platform-brand-search-volume` · 源卡号 `Skill-Cross-Platform-Brand-Search-Volume` · 15-营销投放分析 · Cross-Platform Brand Search Volume — 品牌搜索量作为营销效果领先指标
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-cross-platform-brand-search-volume/SKILL.md`
    ...
    - `p2s-identity-fragmentation-debiasing` · 源卡号 `Skill-Identity-Fragmentation-Debiasing` · 13-广告分析 · Identity Fragmentation Debiasing（身份碎片化纠偏）
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-identity-fragmentation-debiasing/SKILL.md`
    - `p2s-instagram-reels-commerce-attribution` · 源卡号 `Skill-Instagram-Reels-Commerce-Attribution` · 15-营销投放分析 · Instagram Reels Commerce Attribution — Reels 商品发现链路归因与转化量化
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-instagram-reels-commerce-attribution/SKILL.md`
    ...
    - `p2s-organic-paid-rank-synergy-model` · 源卡号 `Skill-Organic-Paid-Rank-Synergy-Model` · 25-搜索流量工程 · 自然排名与广告排名协同效应建模 — 用 Panel DiD 量化广告飞轮 ROI
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-organic-paid-rank-synergy-model/SKILL.md`
    - `p2s-pie-experimental-mta` · 源卡号 `Skill-PIE-Experimental-MTA` · 13-广告分析 · PIE 实验增量预测多触点归因 - Amazon MTA 框架落地
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-pie-experimental-mta/SKILL.md`
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：单位经济、贡献毛利与盈亏平衡是结构化财务建模。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：复盘指标可自动核算与归因，但结论与责任判定需经营判断。
...
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/B/CTR-B-031-申诉材料准备.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：证据整理与材料生成可自动化，但申诉主张与法律后果须人负责。
