# FLOW-05 契约撰写 · 公共材料（所有批次共用）

> 本文件由 `build_contract_workpack.py` 生成，**不要手改**（改了就与图谱脱钩）。
> 逐份契约的规格在 `flow-05-batch-NN.md`。
...
- **FLOW-05** 的能力贡献岗位 14 个：AGT-002 枢衡·场景自主编排与异常协调、AGT-006 听澜·消费者需求与VOC研究、AGT-020 行舟·仓储履约与退货处置、AGT-023 自航·独立站经营与转化、AGT-028 译境·本地化与市场适配、AGT-029 立言·品牌战略与传播、AGT-034 续缘·CRM留存与复购、AGT-035 试真·增长实验与增量评估、AGT-036 安心·售前服务与购买指导、AGT-037 解忧·售后客诉与服务补救、AGT-038 回声·体验洞察与质量反馈、AGT-039 同行·用户教育与会员社区、AGT-040 清账·GMV结算与会计对账、AGT-044 安界·产品合规与隐私
- 8 个阶段（格类型：M = 算法模型接入点，R/D = 只放判据与契约，**不得**说成算法接入）：

| STG | 阶段 | 格 | 业务步骤 | 标准产物（**逐字引用这个名字**） |
|---|---|---|---|---|
| STG-01 | 信号接收 | D | 接收咨询、订单异常、售后或获许可的生命周期事件 | 客户旅程信号 |
| STG-02 | 范围确定与Case创建 | R | 确定客户/订单最小范围、问题类型、许可状态和完成条件 | 客户Case Charter |
| STG-03 | 上下文装配 | D | 按事件类型选择主岗位，装配服务、体验、教育、CRM、隐私和质量能力 | FLOW-05 Context Manifest |
| STG-04 | 证据收集与诊断 | M | 核对客户与订单事实、历史问题、服务规则、触达许可和产品适用性 | 客户问题诊断 |
| STG-05 | 方案与标准产物 | M | 形成答复、补救、教育或复购实验方案 | 服务与留存方案 |
| STG-06 | Assurance接收门禁 | R | 检查许可、隐私、健康风险、质量事件、补偿和触达边界 | FLOW-05 Assurance Decision |
| STG-07 | 受控动作 | D | 提交退款/补救/触达Intent或NoActionRecord | 客户动作尝试或无动作记录 |
| STG-08 | 结果核验、关闭与异步学习 | M | 核对问题解决、补救回执、根因反馈和复购结果 | 客户旅程关闭记录 |
...
- 阶段产物：客户旅程信号 · 客户Case Charter · FLOW-05 Context Manifest · 客户问题诊断 · 服务与留存方案 · FLOW-05 Assurance Decision · 客户动作尝试或无动作记录 · 客户旅程关闭记录
...
- 口径与主数据：AGT-045 指标契约与主数据、AGT-046 的质量状态/来源/新鲜度、
  `业务规则字典`、`标签分配结果表`
- **本 FLOW 的手册 = PB-005**（`docs/06-playbooks/PLAYBOOKS.md`；FLOW↔PB 的对应见 `FLOW-CATALOG.md`）
...
3. 同一句话在本 FLOW 的 41 份里重复出现 ⇒ 视为未撰写。
