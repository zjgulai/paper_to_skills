# FLOW-06 契约撰写 · 公共材料（所有批次共用）

> 本文件由 `build_contract_workpack.py` 生成，**不要手改**（改了就与图谱脱钩）。
> 逐份契约的规格在 `flow-06-batch-NN.md`。
...
- **FLOW-06** 的能力贡献岗位 12 个：AGT-002 枢衡·场景自主编排与异常协调、AGT-003 明镜·经营分析与决策支持、AGT-021 北辰·Amazon业务经营、AGT-023 自航·独立站经营与转化、AGT-040 清账·GMV结算与会计对账、AGT-044 安界·产品合规与隐私、AGT-045 同尺·业务口径与主数据、AGT-046 清源·数据工程与质量、AGT-047 接桥·系统集成与业务工具、AGT-048 积知·知识技能与Playbook治理、AGT-049 稳行·Agent平台与可靠运行、AGT-050 门卫·信息安全与权限
- 8 个阶段（格类型：M = 算法模型接入点，R/D = 只放判据与契约，**不得**说成算法接入）：

| STG | 阶段 | 格 | 业务步骤 | 标准产物（**逐字引用这个名字**） |
|---|---|---|---|---|
| STG-01 | 信号接收 | D | 接收真实业务能力需求或数据错误 | 数据与工具需求信号 |
| STG-02 | 范围确定与Case创建 | R | 确定业务任务、对象、现有方法、优先级、约束和验收结果 | 能力交付Case Charter |
| STG-03 | 上下文装配 | D | 装配口径、数据质量、集成、访问、安全、可靠性和能力治理Skills | FLOW-06 Context Manifest |
| STG-04 | 证据收集与诊断 | M | 核对业务语义、对象映射、数据来源/质量、现有系统和复用可能 | 能力缺口诊断 |
| STG-05 | 方案与标准产物 | M | 形成需求契约、数据契约或工具交付方案 | 数据/工具交付包 |
| STG-06 | Assurance接收门禁 | R | 检查Access、数据质量、运行恢复、安全和经营验收设计 | FLOW-06 Assurance Decision |
| STG-07 | 受控动作 | D | 提交受控发布Change Proposal或NoActionRecord；Bundle变更转入发布生命周期 | 发布尝试或变更提案 |
| STG-08 | 结果核验、关闭与异步学习 | M | 核对业务验收、质量、运行结果并发出异步学习候选 | 能力交付关闭记录 |
...
- 阶段产物：数据与工具需求信号 · 能力交付Case Charter · FLOW-06 Context Manifest · 能力缺口诊断 · 数据/工具交付包 · FLOW-06 Assurance Decision · 发布尝试或变更提案 · 能力交付关闭记录
...
- 口径与主数据：AGT-045 指标契约与主数据、AGT-046 的质量状态/来源/新鲜度、
  `业务规则字典`、`标签分配结果表`
- **本 FLOW 的手册 = PB-006**（`docs/06-playbooks/PLAYBOOKS.md`；FLOW↔PB 的对应见 `FLOW-CATALOG.md`）
...
3. 同一句话在本 FLOW 的 37 份里重复出现 ⇒ 视为未撰写。
