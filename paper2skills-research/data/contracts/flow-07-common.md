# FLOW-07 契约撰写 · 公共材料（所有批次共用）

> 本文件由 `build_contract_workpack.py` 生成，**不要手改**（改了就与图谱脱钩）。
> 逐份契约的规格在 `flow-07-batch-NN.md`。
...
- **FLOW-07** 的能力贡献岗位 19 个：AGT-002 枢衡·场景自主编排与异常协调、AGT-005 守衡·内控审计与独立复核、AGT-011 砺器·硬件结构与材料工程、AGT-012 灵枢·软件算法与应用生态、AGT-013 求证·产品验证与研发项目、AGT-014 择源·OEM供应商开发与协同、AGT-017 衡仓·库存与商品生命周期、AGT-018 质守·生产协同与质量控制、AGT-020 行舟·仓储履约与退货处置、AGT-021 北辰·Amazon业务经营、AGT-027 守店·店铺账号健康与规则、AGT-029 立言·品牌战略与传播、AGT-036 安心·售前服务与购买指导、AGT-037 解忧·售后客诉与服务补救、AGT-038 回声·体验洞察与质量反馈、AGT-041 守金·经营财务与资金、AGT-043 律衡·法务与知识产权、AGT-044 安界·产品合规与隐私、AGT-050 门卫·信息安全与权限
- 8 个阶段（格类型：M = 算法模型接入点，R/D = 只放判据与契约，**不得**说成算法接入）：

| STG | 阶段 | 格 | 业务步骤 | 标准产物（**逐字引用这个名字**） |
|---|---|---|---|---|
| STG-01 | 信号接收 | D | 接收并验证质量、严重客诉或账号重大风险信号；符合D-026候选条件时交由模型外Emergency Guard评估 | 重大事件信号与Guard评估入口 |
| STG-02 | 范围确定与Case创建 | R | 按已发布规则形成唯一FLOW-07 Case Charter；PROTECT或FREEZE时原子写入Case、STG-01/02接收记录与Guard证据 | 重大事件Case Charter与本地原子提交记录 |
| STG-03 | 上下文装配 | D | 按事件类型选择主岗位，装配产品、供应、服务、渠道、法务、合规和安全能力 | FLOW-07 Context Manifest |
| STG-04 | 证据收集与诊断 | M | 核对事件时间线、受影响批次/商品/账号、已发生动作、库存和客户范围 | 事件范围与根因诊断 |
| STG-05 | 方案与标准产物 | M | 形成隔离、处置、客户影响和恢复方案 | 重大事件处置包 |
| STG-06 | Assurance接收门禁 | R | 检查证据、范围、适用策略、外部承诺和恢复条件 | FLOW-07 Assurance Decision |
| STG-07 | 受控动作 | D | 处理正常处置Intent或NoActionRecord；恢复、重新开放、扩大范围、外部承诺、退款、召回、销毁和永久变更只能在本阶段执行 | 正常处置、恢复尝试或无动作记录 |
| STG-08 | 结果核验、关闭与异步学习 | M | 核对紧急保护与正常处置的逐对象权威回执、影响受控、根因和恢复证据 | 重大事件关闭与全动作对账记录 |
...
- 阶段产物：重大事件信号与Guard评估入口 · 重大事件Case Charter与本地原子提交记录 · FLOW-07 Context Manifest · 事件范围与根因诊断 · 重大事件处置包 · FLOW-07 Assurance Decision · 正常处置、恢复尝试或无动作记录 · 重大事件关闭与全动作对账记录
...
- 口径与主数据：AGT-045 指标契约与主数据、AGT-046 的质量状态/来源/新鲜度、
  `业务规则字典`、`标签分配结果表`
- **本 FLOW 的手册 = PB-007**（`docs/06-playbooks/PLAYBOOKS.md`；FLOW↔PB 的对应见 `FLOW-CATALOG.md`）
...
3. 同一句话在本 FLOW 的 55 份里重复出现 ⇒ 视为未撰写。
