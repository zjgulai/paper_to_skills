...
| 域 / 面 | `DOM-08` / `PLN-PLT` |
| flows | FLOW-06 |
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/A/CTR-A-068-数据质量.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 A 类（⚠️ 来源＝本项目《经营侧组织模型精确抽取》`reports/_survey_org_model.md` §F.3 的判定，**二手来源，不是材料原文**，别改判）**：数据质量规则、分布漂移与缺失检测是标准数据质量统计方法。
> 引用纪律：这句判定可以进正文，但**不得标成「材料 §F.3 原文」**。S1 实测 139 份作业包全都
> 把它标成了「材料 §F.3 原文」，撰写人于是写出「材料 §F.3 原文写明『流程合理性须业务确认』」
> 这类**把我们的分析记到材料账上**的句子 —— 核验器实测该句在材料里 0 命中。
...
- 只有 legacy 预览版（可引 slug，但 §1 必须写明「论文实验参数未随卡进入本包」，清单按卡面可见数字列）：共 50 张
    - `p2s-ai-generated-content-detection` · 源卡号 `Skill-AI-Generated-Content-Detection` · 11-AI人文 · AI 生成内容检测 — Perplexity/Burstiness 统计特征分类器
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-ai-generated-content-detection/SKILL.md`
    ...
    - `p2s-ragas-rag-evaluation-framework` · 源卡号 `Skill-RAGAS-RAG-Evaluation-Framework` · 08-知识图谱 · RAGAS — RAG 质量自动化评估框架
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-ragas-rag-evaluation-framework/SKILL.md`
    ...
    - `p2s-realtime-feature-collection` · 源卡号 `Skill-Realtime-Feature-Collection` · 22-数据采集工程 · Realtime Feature Collection — 流式特征采集与在线特征仓库：推荐系统实时个性化的数据基础设施
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-realtime-feature-collection/SKILL.md`
    ...
    - `p2s-sdpm-semantic-chunking` · 源卡号 `Skill-SDPM-Semantic-Chunking` · 08-知识图谱 · SDPM语义双阶段分块 — 边界感知的智能分块策略
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-sdpm-semantic-chunking/SKILL.md`
    - `p2s-stamimputer-spatiotemporal` · 源卡号 `Skill-STAMImputer-SpatioTemporal` · 14-用户分析 · 时空注意力混合专家补全 - 高缺失率下的多维流量恢复
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-stamimputer-spatiotemporal/SKILL.md`
- 仅精选线（尚未换底进产品线，引 `card_id`，并注明「S5 换底后替换为已装 slug」）：共 1 张
    - `Skill-Review-Quality-Scoring` · 07-NLP-VOC · Skill-Review-Quality-Scoring
      - 全文卡：`paper2skills-vault/07-NLP-VOC/00-知识库-Skill卡片/Skill-Review-Quality-Scoring.md`

> 候选总数 52。**`cards` 只写 1–3 张代表卡**（同族，不是全部）——
> 契约按**责任**建、不按卡建，§1 必须写一句「本契约对同族方法卡通用；换卡时只替换方法实现，
> 其余要求不变」。
