...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：专利/商标检索与相似度、侵权风险打分是标准IR与文本相似建模。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：产能与节拍可建模（排队论/瓶颈分析），但输入依赖供方声明，需审核验证。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：版本比对与回归评测门禁是标准MLOps评估工程，但属制品治理管道。
