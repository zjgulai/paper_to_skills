...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：多维GMV拆解与增量归因有成熟方法（Shapley/Markov归因、MMM、因果森林），输出可归因的贡献分解。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：ABC/XYZ分层与安全库存分位计算是标准库存分析，可直接产出分层与目标库存。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：查询意图与相关性是标准IR/查询理解与词向量建模问题。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：价格弹性可用需求模型与因果实验（价格A/B、DML）直接估参。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：广告诊断是归因分解与异常检测的直接应用（含预算受限与缺货联动）。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：收入确认与费用归集可用规则引擎加统计异常检测核对。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：跨账号/平台SKU映射是实体解析（entity resolution）与相似匹配问题。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：渠道可比性可用多源数据与基准模型评估，但结论依赖外部数据可得与实地验证。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：文化/合规风险可用多语分类器预筛，但当地理解须本地证据。
