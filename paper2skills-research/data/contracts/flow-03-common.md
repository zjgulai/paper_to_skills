# FLOW-03 契约撰写 · 公共材料（所有批次共用）

> 本文件由 `build_contract_workpack.py` 生成，**不要手改**（改了就与图谱脱钩）。
> 逐份契约的规格在 `flow-03-batch-NN.md`。
...
- **FLOW-03** 的能力贡献岗位 13 个：AGT-002 枢衡·场景自主编排与异常协调、AGT-014 择源·OEM供应商开发与协同、AGT-015 契约·采购与合同履约、AGT-016 知量·需求预测与补货计划、AGT-017 衡仓·库存与商品生命周期、AGT-018 质守·生产协同与质量控制、AGT-019 通途·跨境物流与关务、AGT-020 行舟·仓储履约与退货处置、AGT-021 北辰·Amazon业务经营、AGT-026 衡价·定价促销与商品组合、AGT-040 清账·GMV结算与会计对账、AGT-041 守金·经营财务与资金、AGT-042 合账·税务与跨境实体协作
- 8 个阶段（格类型：M = 算法模型接入点，R/D = 只放判据与契约，**不得**说成算法接入）：

| STG | 阶段 | 格 | 业务步骤 | 标准产物（**逐字引用这个名字**） |
|---|---|---|---|---|
| STG-01 | 信号接收 | D | 接收库存、预测、交期、采购或履约变化 | 供需信号 |
| STG-02 | 范围确定与Case创建 | R | 确定账号、SKU/商品、仓库、供应商、时间窗和供需目标 | 供需Case Charter |
| STG-03 | 上下文装配 | D | 装配预测、库存、采购、OEM、物流、财务与质量能力 | FLOW-03 Context Manifest |
| STG-04 | 证据收集与诊断 | M | 核对可售/预留/在途、已有订单、需求区间、交付周期、产能、条款和现金 | 供需证据与约束诊断 |
| STG-05 | 方案与标准产物 | M | 比较补货、调拨、控投放和清货方案 | 供应计划与动作方案 |
| STG-06 | Assurance接收门禁 | R | 检查重复订单、MOQ、产能、现金、质量、合同和对象状态 | FLOW-03 Assurance Decision |
| STG-07 | 受控动作 | D | 提交采购/调拨Intent或NoActionRecord；具体策略和ERP schema待配置 | 执行尝试或无动作记录 |
| STG-08 | 结果核验、关闭与异步学习 | M | 核对订单、到货、质检、可售、结算和资金影响 | 供需关闭记录 |
...
- 阶段产物：供需信号 · 供需Case Charter · FLOW-03 Context Manifest · 供需证据与约束诊断 · 供应计划与动作方案 · FLOW-03 Assurance Decision · 执行尝试或无动作记录 · 供需关闭记录
...
- 口径与主数据：AGT-045 指标契约与主数据、AGT-046 的质量状态/来源/新鲜度、
  `业务规则字典`、`标签分配结果表`
- **本 FLOW 的手册 = PB-003**（`docs/06-playbooks/PLAYBOOKS.md`；FLOW↔PB 的对应见 `FLOW-CATALOG.md`）
...
3. 同一句话在本 FLOW 的 36 份里重复出现 ⇒ 视为未撰写。
