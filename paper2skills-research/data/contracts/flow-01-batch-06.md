...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：需求预测是教科书级时序/层级/概率预测问题（含促销与新品冷启动）。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：渠道诊断可用流量-转化-客单分解与增量归因模型直接产出偏差定位。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：转化优化是A/B、多臂老虎机与推荐排序优化的直接应用。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：账号健康与违规风险可用异常检测与风险评分模型持续监测。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：实验设计（功效、分配、序贯、CUPED）是标准统计方法。
...
**本责任为什么是 A 类（材料 §F.3 原文，别改判）**：预算编制与滚动预测是约束优化与偏差追踪问题。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：异常检测可用统计/时序离群方法，但冻结范围与恢复门禁是确定性治理规则，模型不得选参。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：政策变化可抓取与差异对比，但影响解读与合规动作须人确认。
...
**本责任为什么是 B 类（材料 §F.3 原文，别改判）**：假设可形式化审计（SUTVA/溢出/混杂诊断），但结论采信须评审判断。
