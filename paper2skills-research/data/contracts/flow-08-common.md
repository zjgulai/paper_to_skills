# FLOW-08 契约撰写 · 公共材料（所有批次共用）

> 本文件由 `build_contract_workpack.py` 生成，**不要手改**（改了就与图谱脱钩）。
> 逐份契约的规格在 `flow-08-batch-NN.md`。
...
- **FLOW-08** 的能力贡献岗位 17 个：AGT-001 衡远·经营目标与资源统筹、AGT-002 枢衡·场景自主编排与异常协调、AGT-003 明镜·经营分析与决策支持、AGT-004 知人·组织能力与人事行政、AGT-005 守衡·内控审计与独立复核、AGT-008 拓新·产品组合与新品孵化、AGT-021 北辰·Amazon业务经营、AGT-029 立言·品牌战略与传播、AGT-035 试真·增长实验与增量评估、AGT-040 清账·GMV结算与会计对账、AGT-041 守金·经营财务与资金、AGT-042 合账·税务与跨境实体协作、AGT-043 律衡·法务与知识产权、AGT-045 同尺·业务口径与主数据、AGT-048 积知·知识技能与Playbook治理、AGT-049 稳行·Agent平台与可靠运行、AGT-050 门卫·信息安全与权限
- 8 个阶段（格类型：M = 算法模型接入点，R/D = 只放判据与契约，**不得**说成算法接入）：

| STG | 阶段 | 格 | 业务步骤 | 标准产物（**逐字引用这个名字**） |
|---|---|---|---|---|
| STG-01 | 信号接收 | D | 接收经营复盘节奏、重大目标偏差或能力退化信号 | 经营复盘信号 |
| STG-02 | 范围确定与Case创建 | R | 确定经营范围、观察窗、资源/能力问题和完成条件 | 经营复盘Case Charter |
| STG-03 | 上下文装配 | D | 装配经营分析、增量、财务、审计、组织、知识和运行能力 | FLOW-08 Context Manifest |
| STG-04 | 证据收集与诊断 | M | 核对经营结果、全成本、自治异常、新品验证和能力表现 | 经营与能力诊断 |
| STG-05 | 方案与标准产物 | M | 形成资源调整、停止事项、策略或能力Change Proposal | 经营与能力变更建议 |
| STG-06 | Assurance接收门禁 | R | 检查口径、现金、利益冲突、独立审计和观察窗口 | FLOW-08 Assurance Decision |
| STG-07 | 受控动作 | D | 提交已有策略范围内的资源动作或Change Proposal；Bundle只能走D-023生命周期 | 受控动作或无动作记录 |
| STG-08 | 结果核验、关闭与异步学习 | M | 核对资源/能力结果、观察窗和回退证据 | 经营复盘关闭记录 |
...
- 阶段产物：经营复盘信号 · 经营复盘Case Charter · FLOW-08 Context Manifest · 经营与能力诊断 · 经营与能力变更建议 · FLOW-08 Assurance Decision · 受控动作或无动作记录 · 经营复盘关闭记录
...
- 口径与主数据：AGT-045 指标契约与主数据、AGT-046 的质量状态/来源/新鲜度、
  `业务规则字典`、`标签分配结果表`
- **本 FLOW 的手册 = PB-008**（`docs/06-playbooks/PLAYBOOKS.md`；FLOW↔PB 的对应见 `FLOW-CATALOG.md`）
...
3. 同一句话在本 FLOW 的 47 份里重复出现 ⇒ 视为未撰写。
