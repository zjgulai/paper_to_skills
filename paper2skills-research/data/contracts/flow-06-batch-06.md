...
| 域 / 面 | `DOM-08` / `PLN-PLT` |
| flows | FLOW-06 |
| 骨架文件 | `paper2skills-vault/07-资源库/contracts/B/CTR-B-059-接口契约.md` |
| `status` 初值 | **可写**（有候选卡） |

**本责任为什么是 B 类（⚠️ 来源＝本项目《经营侧组织模型精确抽取》`reports/_survey_org_model.md` §F.3 的判定，**二手来源，不是材料原文**，别改判）**：契约生成与兼容性检查可工具化，但语义与责任边界须双方确认。
> 引用纪律：这句判定可以进正文，但**不得标成「材料 §F.3 原文」**。S1 实测 139 份作业包全都
> 把它标成了「材料 §F.3 原文」，撰写人于是写出「材料 §F.3 原文写明『流程合理性须业务确认』」
> 这类**把我们的分析记到材料账上**的句子 —— 核验器实测该句在材料里 0 命中。
...
- 只有 legacy 预览版（可引 slug，但 §1 必须写明「论文实验参数未随卡进入本包」，清单按卡面可见数字列）：共 13 张
    ...
    - `p2s-schema-evolution-data-contract` · 源卡号 `Skill-Schema-Evolution-Data-Contract` · 22-数据采集工程 · Schema Evolution & Data Contract — 数据契约与 Schema 演化管理
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-schema-evolution-data-contract/SKILL.md`
    - `p2s-skill-card-api-serving` · 源卡号 `Skill-Skill-Card-API-Serving` · 16-智能体工程 · Skill Card API Serving — 将 Skill 代码模板包装为参数化 REST 微服务
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-skill-card-api-serving/SKILL.md`
    - `p2s-tool-auto-discovery` · 源卡号 `Skill-Tool-Auto-Discovery` · 16-智能体工程 · Tool Auto Discovery — Agent 工具自动发现：OpenAPI + MCP Schema 自注册
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-tool-auto-discovery/SKILL.md`
    - `p2s-tool-call-decision-framework` · 源卡号 `Skill-Tool-Call-Decision-Framework` · 16-智能体工程 · Tool Call Decision Framework — 必要性/效用/可负担性三维工具调用决策
      - 注意：**legacy 预览版**（八段式：④/⑤ 段是占位串、代码以 `references/implementation.py` 随卡且为节选、⑧ 论文出处常写「未自动抽取」或「待人工判定」）→ `/Users/lute/.dsh/skills/p2s-tool-call-decision-framework/SKILL.md`
