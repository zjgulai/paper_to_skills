...
| ps_override entries | **1034** (100% coverage) |
...
│           └── config/
│               ├── skill_ps_override.yaml     # problem_solved 覆盖（1034条，100%覆盖）
...
| `/skills/` | 全部 Skills (1034个) | 每 Skill 独立详情页 |
...
**位置**：`paper2skills-skills/diagnostic-sop/diagnose.py`

任何人遇到业务问题，**30 分钟内**跑完三层诊断（诊断根因 / 处置行动 / 预防机制）。
...
**支持问题类型**：`repurchase_drop` / `roas_decline` / `traffic_drop` /
`new_cvr_low` / `inventory_issue` / `review_attack`

**四个诊断模块**：VOC评价分析 / 竞品价格监控 / 婴儿月龄时钟（母婴专属）/ 因果归因

**输出**：三层报告（诊断/处置/预防）+ 每步行动含 Skill 直达 URL
