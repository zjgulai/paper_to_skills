...
| `paper2skills-research/scripts/domains.py` + `candidate_filter.py` + `migrate_domain_labels.py` + `check_pipeline_chain.py` + `check_keyword_tables.py` | **域名唯一事实源 + 三段式过滤 + 三态判定 + 第 2 环接线 + 词表三向对账（P1→P3）；验收面 L18a–L18f · L19a–L19f · L20a–L20d**。**P1**：域名曾 **7 处各写一份**，`if not domains: keep` 把标签认不出的论文**整篇放行** —— 1046 篇里 **141 篇**的负向词与约束词一条都没生效且**无任何读数**（修后判定 905→**1046**）。**P2**：约束词「显著性」假设被实测推翻（代价 9 篇不是 4 篇），交付**证据形态五态**为一等读数，**判定一字不改**。**P2b**：语法拆表判据全废 ⇒ 成立的是**实测域精度 > 0.5**（召回 18/26→**26/26**、标出 68→**42**，仍是分诊读数）；**7/15 域整表泛词 ⇒ 读数为空**；窗口仅 **[0.4,0.5]**。⭐ **P2c 接线（#105）**：过滤器此前**根本没有下游** ⇒ 已接通 `1046 → 772`（丢 274），`dropped_ids` 逐条留名。三条不许松的纪律（**各有能打红的负向控制**）：① 过滤产物读不到 ⇒ **exit 2**（**无静默回退**）；② 判据看**自述块 `filter`** 不看文件名（改名冒充 ⇒ 照样 exit 2）；③ `unjudged_ids` 非空 ⇒ **exit 2**。反向控制：剔墙钟项（#103）后 772 条**零漂移**。⚠️ 同批撞出四雷 **#113–#116**（死脚本两天而门禁无一跑过它 · 一次性初始化器重跑会毁 **39/45** 条 ⇒ 默认只对账不写盘 · 主控复核假绿 · 判据只认一种写法判假红）。新增 **L19e/L19f**（`check_pipeline_chain.py`）。⭐ **P3 词表三向对账**：三段式检索词住在**三处而没有任何门禁比对过**（散文 §2 / `QUERY_GROUPS` / `candidate_filter` 两张表）⇒ §2 的 **180** 个正向词只有 **74** 个进过收割查询（如 01 的 `difference-in-differences`）；**`10-MAS`/`11-AI人文` 代码里没有约束表**（按散文表各丢 **0/82**、**21/21** —— 后者正是散文预言的失败模式）；274 条丢弃**四分**（`OWN_TOPICAL_DROPPED` 210 / `RESCUE_TOPICAL` **5** / `CONTEXT_ONLY` 50 / `NO_EVIDENCE` 9）；P2c 那句「226 条换域可过」复算 **226/226** 后**拆开**：`226 = 主题证据 13 + 纯上下文 213` ⇒ **94.2% 不携带归属信息，不是错杀证据**。新增 **L20a–L20d**（`check_keyword_tables.py`）。详见 `reports/PHASE6-P2c-第2环接线.md` 与 `reports/PHASE6-P3-词表三向对账与274条裁决.md`（P1/P2/P2b 见同名系列报告） |
...
| `paper2skills-research/scripts/check_feishu_residue.py` + `strip_feishu_from_playbook.py` + `data/feishu-residue-baseline.json` | **飞书残留门禁与剥离器（2026-09-14，验收面 L17a/L17b/L17c）**：按所有者要求把飞书 webhook 连同飞书相关内容移出本项目（1496 页生成物 + sync 目标 + 文档）。**playbook/ 是上游构建产物、本仓库没有生成器** ⇒ 剥掉 ≠ 不会回来，这种回归**没有别的门禁看得见**。**判据 A（判红）**：10 个集成标识必须全 0，**清单唯一一处定义在门禁里、剥离器 import 它**（风险 N2）；**判据 B（只判增长）**：裸词普查 vs baseline，收缩点名提示收紧（#84）；豁免**逐文件 × 逐标识**且每次跑都打印（#78）。剥离器 11 种形态 `--selftest` **19/19** · 门禁 `--selftest` **12/12**。⚠️ **删代码杀不掉那条暴露的 webhook**：它仍活、仍能从公开历史一行还原 ⇒ L16a 照旧判红，闭环只有飞书后台轮换。自己撞出的三个缺陷（#92 删块必须花括号配对 / #93 两次「只认一种写法」/ #94 判据把事故记录判成残留）与**仍在上游的**两个服务端路由见 **`reports/PHASE6-F8-S13-飞书移除.md`** |
...
**远端只有一个**:https://github.com/zjgulai/paper_to_skills (**PUBLIC**)。⚠️ **本项目只推这一个远端**（所有者 2026-09-13 决定）：**不发 CodeUp、不加任何镜像远端**；`git remote -v` 应当只有 `origin` —— 多出来的远端要问清再推，别自作主张做镜像。
...
> ⚠️ **本机推 GitHub 必须走代理**（2026-09-13 实测）：`127.0.0.1:7890` 有代理在听、macOS 系统代理也指着它，但 **`curl`/`git` 都不读系统代理**且 `http_proxy` 为空 ⇒ 症状是「DNS 正常、baidu 200、GitHub 全系 000 超时」，看着像网络故障，**其实是没人用那个代理**。修法 `git -c http.proxy=http://127.0.0.1:7890 push origin main`（`gh` 用 `HTTPS_PROXY`）。⚠️ 别写死进 `~/.gitconfig`：**别的项目走直连就通**，全局代理会连它们一起绕出去。
