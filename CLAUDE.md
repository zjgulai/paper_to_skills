...
| `paper2skills-research/scripts/run_phase6_gates.py` | **PHASE6 验收面（2026-09-13）**：**一条命令跑完 19 条门禁**，也是本阶段第一次有「整体状态」这个读数 —— 此前 11 个门禁脚本分散在三种命令形态里，每个数字只活在产它的那份报告里（CLAUDE.md 记着「三个门禁必须分开报」，而分开报的前提是**有一个地方能把它们放在一起报**）。三条硬纪律：① **退出码不许合并成一个分数**，逐门禁给码、汇总只给各类几条，危险性排序 **3 > 2 > 1 > 0**（「没测到」比「测了是红的」更危险：红会有人去修，没测到没人知道）；② **一个门禁都没跑到 = exit 3，不是 0**（同 `scan_secrets.py`：扫到 0 个文件判失败）；③ **豁免必须可见、带到期条件**，且**转绿时提示「请取消豁免」**（永久豁免＝台账 #5 那种腐烂）。`--only` / `--fast` / `--list` / `--json-out`。`--selftest` **11/11**（含 ④–⑧ 用假门禁验证退出码**传播**：判红⇒1、跑不起来⇒**2 不是 0**、混合⇒取最危、空列表⇒**3**）；`--mutate` **3/3** 把 `check_material_citations.py` 改坏，验证端到端用例有劲。⚠️ **它自己被抓出两个缺陷**（都是 `--mutate` 干的，这是本文件最该留的部分）：首版 `cli()` 硬编码了原文件路径 ⇒ **变异体从没被跑过**，而读数看起来像「用例是摆设」；首版用裸 `    if bad:` 做变异锚点，而该文本在文件里出现两次（打印分支在前、退出码分支在后），`replace(...,1)` 只改到打印那处 ⇒ 变异没施上力。**先证明变异改变了真实取值，再谈判据有没有劲**（同 S4 缺陷⑥）。另有一条 `... or True` 的恒真断言被改成真断言（本仓库第 4 次抓到这一形态）。**实测读数（2026-09-13）**：19 条 ✅16 · 🔴3（L4e 材料引文待清 · L5a 已豁免 · L5b S13 施工中） |
