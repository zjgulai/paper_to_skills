...
| `check_material_residue.py` | **材料归属残留器（S1 收口期，#67 交付；#78 接线；W-67c 收口）**：引文器只看「同行同分句」，它把颗粒度放宽到**blockquote 条目块 + 括号跨度**，专门抓续行/枚举形态的漏网。**两族分开判，现在两族都真判**：**家族二**（`材料 §X`，材料没有该编号 —— 材料编号只有 `1–12`/`R01–R05`，`§E.`/`§F.` 是**我方综述**的章节号）⇒ **L4k**；**家族一**（块内声称材料而词查无实据）⇒ **L4m**。改标补丁两份：`patch_material_attribution.py`（家族二，表驱动 40 锚点）与 `patch_material_quarter_tier.py`（**W-67c**：`「季度经营策略」`**37 处搬家**（从 §0「(a) 材料已给出的数」搬到 (b)「业务侧默认」）+ **7 处正文插入** + **3 处改述**；参数 `--check`/`--apply`/`--selftest`）。读数：家族二 **38 → 0**，家族一 **66 → 41 → 4 处**（37 处「季度经营策略」全部改标，余 4 处**逐处复核为假阳性**：`材料` 作普通名词 / 契约自述 ⇒ 走 **`data/material-residue-baseline.json` 可见豁免**）。`--selftest` **21/21**（含家族开关**双向**隔离 · **⑯ 两个脚本 `normalize()` 逐字节相等** · ⑲/⑳ 去归属语查词自己身边一正一反 · **㉑–㉔ baseline 双向**：豁免生效 / 只豁免点名那一处 / **过期豁免判红** / 不传时逐字同判）· 端到端 5 用例 · 变异 **R1–R4 全抓住**。⚠️ **#79**：两个脚本被注释声明为「同尺」**实际不同尺** ⇒ 家族一 20 个词里 **13 个是假阳性**（已改为 `import` 引文器的 `normalize`）。⚠️ **#80**：去归属语只查 `材料` 附近不查**词自己**附近 ⇒ 4 处假阳性（`材料已给出的数（…）` 的括号是**语法容器**，容器里每一项**自带归属**）。⚠️ **#84**：豁免的提示若只 printf，**绿门禁的 stdout 根本不显示** ⇒ **过期豁免必须判红**。⚠️ **#82（纪律改写）**：补丁纪律从「**只做插入**」改为「**所删必现于所增，改述必须逐处点名**」—— 「不删一字」写成了代理指标 ⇒ 逼出嵌套括号 / 粘字 / **标了归属却仍留着错话**三种坏结构。详见 **`reports/PHASE6-F8-S1W-材料归属改标与残留门禁.md`** |
