...
| `paper2skills-research/scripts/domains.py` + `candidate_filter.py` + `migrate_domain_labels.py` | **域名的唯一事实源 + 三态过滤 + 一次性迁移（P1）+ 约束词证据形态与规格对账（P2）；验收面 L18a–L18f · L19a–L19c**。**P1**：域名曾 **7 处各写一份**，`if not domains: keep` 把标签认不出的论文**整篇放行** —— 1046 篇里 **141 篇**的负向词与约束词一条都没生效且**无任何读数**（修后 判定 905→**1046**、保留 776→**772**、静默放行→**0**）。canonical 以 **vault 顶层目录**双向对账；`RETIRED` **不参与解析**（认得它＝替漂移兜底＝取消发现漂移的能力）；「仪器瞎了」⇒ **exit 2**；迁移**不重跑生成器**（分数含墙钟项）。**P2**：上一批登记「补 `text-to-sql` 会放进 4 篇顺带提及的论文，区分点是**标题 vs 摘要的显著性**」——逐条读原文后**代价是 9 篇不是 4 篇**（28 条**全体普查**，落 `data/constraint-salience-labels.json`），而**两个机械形式都被实测推翻**：字面版砍 **326/597** 个仅摘要保留对；深度版全池标 68 条却**漏 3/9 真污染 + 62 条假阳性**（被同一表里的泛词 `table` 救回 —— 混合词表里一个泛词就能救活主题埋在八成处的论文）。⇒ 交付**证据形态五态**为**一等读数**，**判定一字不改**，「不许据此丢弃」由 selftest 守卫 + 变异 M10 常驻守。⚠️ **L19c 查出 `关键词库-v2.md` §0 点名的两个函数全仓只出现一次（就在那行），且过滤器产物无消费者** ⇒ **第 2 环是断的**（台账 **#105**，接线为独立工单）。**P2b（拆表）**：三个**语法**判据全废，成立的是**实测域精度 > 0.5** —— 污染召回 **18/26 → 26/26**、标出 **68 → 42**。⚠️ 只是**分诊读数**（10 条真命中、`judge()` 未改）· **7/15 域整表泛词 ⇒ 读数为空** · 阈值窗口只有 **[0.4,0.5]**。验收面 **L19d**；台账 #110–#112。详见 `reports/PHASE6-P2b-约束词拆表.md` |
