...
| v2 必填字段实际欠账 | `paper_id` 8 / `paper` 29 / `venue` 66 / `venue_tier` 74 / `evidence_grade` 78(共 96 张有来源卡) | ✅ **2026-09-13 起 C2 已跟进口径**(X3b):126 → **78**,与上列五项**逐项一致**;另单列 48 张 `author-practice` 豁免与名单。⚠️ 遗留口径问题:**11 张卡 frontmatter 用 `name:` 而非 `title:`**(2 张是 skill 格式的 name+description),C2 因此把它们算作缺 `title` —— 与漏洞 #11 同源(*判据只认一种字段名*) |
...
| 仓库体检欠账 | C3 🟡16 / C8 🟡58 | C1/C4/C5/C6/C7 ✅;**C2 口径失真已于 2026-09-13 修复**(X3b)。⚠️ C2 原先**没有 `total` 字段**,于是**有 126 张欠账时仍显示 ✅**(图标只看 critical/total)—— 补上后如实显示 🟡 |
