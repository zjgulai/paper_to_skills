...
| `paper2skills-research/scripts/build_gap_ledger.py` | **缺口账与靶区工单（PHASE6 F3 起）**：把 151 责任名 × A/B/C × 供给状态**算出来**（不抄文档）并与方案 §4 逐格断言相等（J1–J5）；产物 `data/gap-ledger.json` + `reports/PHASE6-F3-缺口账与靶区工单.md`（36 条工单按位次排序）。排序权重**只用于排序**（J7 扫 8 个门禁脚本 + 3 种注入样本自证，风险 N1）。⚠️ 首跑抓到材料矛盾：`产能调查` §F.5 记 A、§F.3/§F.4 判 B ⇒ 登记不重判。详见方案 §4.1 |
