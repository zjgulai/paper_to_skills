...
| `paper2skills-research/scripts/domains.py` + `candidate_filter.py` + `migrate_domain_labels.py` + `check_pipeline_chain.py` | **域名唯一事实源 + 三段式过滤 + 三态判定 + 第 2 环接线（P1→P2c）；验收面 L18a–L18f · L19a–L19f**。**P1**：域名曾 **7 处各写一份**，`if not domains: keep` 把标签认不出的论文**整篇放行** —— 1046 篇里 **141 篇**的负向词与约束词一条都没生效且**无任何读数**（修后判定 905→**1046**、静默放行→**0**）。**P2**：约束词「显著性」假设被实测推翻（代价 9 篇不是 4 篇），交付**证据形态五态**为一等读数，**判定一字不改**。**P2b**：三个**语法**拆表判据全废，成立的是**实测域精度 > 0.5**（污染召回 18/26→**26/26**、标出 68→**42**）—— 仍是**分诊读数**，`judge()` 未改；**7/15 域整表泛词 ⇒ 读数为空**；阈值窗口只有 **[0.4,0.5]**。⭐ **P2c 接线（#105）**：过滤器此前**根本没有下游** ⇒ 现已接通，`1046 → 772`（丢 274），`dropped_ids` 逐条留名。三条不许松的纪律（**各有能打红的负向控制**）：① 过滤产物读不到 ⇒ **exit 2**，**没有静默回退**；② 判据看**自述块 `filter`** 而不是文件名（未过滤池改名冒充 ⇒ 照样 exit 2）；③ `unjudged_ids` 非空 ⇒ **exit 2**。⚠️ 反向控制：剔掉**墙钟项**（#103）后 772 条保留条目**零漂移**（接线只改集合、不改判分）。⚠️ 同批在链上撞出三颗雷：**#113** `build_registry.py` 模块级前向引用 ⇒ **死脚本两天而 65 条门禁无一跑过它**（「接线」也不是「跑得起来」）；**#114** 它是**一次性初始化器**，重跑会毁 **39/45** 条人工核对的记录 ⇒ 默认**只对账不写盘**；**#115** 主控第一版复核是**假绿**（脚本没写盘，比的是同一份文件）；**#116** J3 读者判据**只认一种写法**，对着刚接好的线判了**假红**（现认三种写法 + argparse 变量名不写死，8 例全进 selftest）。新增 **L19e/L19f**（`check_pipeline_chain.py`：加载探针 · 第 2 环三条纪律 + 反向控制 · 初始化器不改写事实源 · **门禁自身不改动真产物**）。详见 `reports/PHASE6-P2c-第2环接线.md`（P1/P2/P2b 见同名系列报告） |
