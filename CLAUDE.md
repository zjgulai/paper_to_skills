...
| `paper2skills-research/scripts/domains.py` + `candidate_filter.py` + `migrate_domain_labels.py` | **域名的唯一事实源 + 三态过滤 + 一次性迁移（PHASE6 P1，2026-09-14；验收面 L18a–L18f）**。守的是一类**别的门禁都看不见的失效**：域名字符串曾 **7 处各写一份**且互不相同，而 `filter_pool` 的 `if not domains: keep` 把标签认不出的论文**整篇放行** —— 实测 1046 篇里 **141 篇**的负向词与约束词**一条都没生效且没有任何读数**（修后：判定 905→**1046**、保留 776→**772**、静默放行→**0**）。canonical 以 **vault 顶层目录**为准、双向对账；`RETIRED`（退休名）**不参与解析**（认得它＝替漂移兜底＝取消发现漂移的能力）；过滤器三态中「仪器瞎了」⇒ **exit 2**。迁移**不重跑生成器**（`rank_candidates` 分数含**墙钟项**，会把时间漂移混进「改域名」这个提交）。`--mutate` 各 8/8，且**8/8 红在正确的判据上**。详见台账 **#99–#104** |
