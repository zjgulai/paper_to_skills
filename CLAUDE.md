...
| `check_material_residue.py` | **材料归属残留器（S1 收口期，#67 交付；#78 才接线）**：引文器只看「同行同分句」，它把颗粒度放宽到**blockquote 条目块 + 括号跨度**，专门抓续行/枚举形态的漏网。**两族分开判、且只接了一族**：**家族二**（`材料 §X`，材料没有该编号 —— 材料编号只有 `1–12`/`R01–R05`，`§E.`/`§F.` 是**我方综述**的章节号）已逐号核实 ⇒ **L4k**；**家族一**（块内声称材料而词查无实据）**未定标 ⇒ 只扫不判**（`--family {all,terms,sections}`，未判的那一族**明写「只扫不判」**：未判定 ≠ 干净）。改标补丁 `patch_material_attribution.py`（表驱动 40 锚点、每处断言恰好命中一次、**不删一字**；三态判定「待应用/已应用/过期对不上」）。读数：家族二 **38 → 0**（23 份），家族一 66 → 63 处（**未清**，见 W-67b/c）。`--selftest` **12/12**（含家族开关**双向**隔离 + 「材料真有该号不算残留」）· 端到端 5 用例 · 变异 **R1–R4 全抓住**。详见 **`reports/PHASE6-F8-S1W-材料归属改标与残留门禁.md`** |
