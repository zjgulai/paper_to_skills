...
本地存储路径：
```
<REPO_ROOT>/paper2skills-vault/[领域]/Skill-[算法名称].md
```
...
代码存储路径：
```
<REPO_ROOT>/paper2skills-code/[领域]/[算法]/
├── __init__.py
├── model.py
└── example.py
```
...
python <REPO_ROOT>/paper2skills-skills/paper-同步/scripts/sync.py --skill Skill-Uplift-Modeling.md --target vault,github

# 查看同步状态
python <REPO_ROOT>/paper2skills-skills/paper-同步/scripts/sync.py --skill Skill-Uplift-Modeling.md --status

# 查看所有同步状态
python <REPO_ROOT>/paper2skills-skills/paper-同步/scripts/sync.py --status
```
...
```
状态记录路径：`<REPO_ROOT>/paper2skills-vault/07-资源库/sync_status.json`
```
