...
TOB_PLAYBOOKS = [
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Epidemiological-Viral-Traffic-SIR", "why": "SIR流行病学模型提前2周预测TikTok爆款流量拐点，精准控制补货量，防止高位接盘死库存（ROI 80-150万）"},
                    {"id": "Skill-TikTok-Flash-Sale-Inventory-Pulse", "why": "TikTok闪购触发库存脉冲：爆款信号→自动触发紧急备货指令，响应时间从48h→4h"},
                ],
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Promo-Inventory-Pulse-Auto-Trigger", "why": "大促库存脉冲自动触发：实时销速×剩余库存天数达到阈值，自动触发紧急补货和仓间调拨（断货率降低40%）"}],
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Abandoned-Cart-Recovery-Trigger", "why": "弃购找回触发器：基于弃购时长+商品类型+用户历史，精准选择邮件/短信/广告再营销渠道（弃购找回率提升18-35%）"}],
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Post-Purchase-Review-Request-Dispatcher", "why": "购后评价请求分发：根据用户满意度预测分数决定何时请求评价、哪种渠道，避免差评风险（评价数量提升25%，避免低星评价）"}],
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-OOS-Emergency-Airfreight-Gate", "why": "断货应急空运门控：DOS < 7天时自动触发紧急空运决策，防止大促期间断货损失（ROI 30-80万/次）"}],
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Nonlinear-Multi-Touch-Attribution", "why": "非线性多触点归因：基于Transformer的序列建模替代线性归因，捕捉TikTok-搜索-购买的非线性交互（ROI 200-400万）"}],
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-CASE-Cadence-Aware-Repurchase-Prediction", "why": "节奏感知复购预测：捕捉母婴月龄节律（宝宝3月→6月→12月的产品需求切换），在正确时间窗口触达（ROI 40-80万）"}],
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Subscription-Renewal-Intervention-Gate", "why": "订阅续费干预门控：临近订阅到期且流失风险>60%时自动推送专属续费激励，续费率提升15-25%"}],
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Brand-Keyword-Hijack-Alert", "why": "品牌词劫持告警：实时检测竞品Listing出现在品牌词搜索结果中，自动触发防御性竞价和投诉（保护品牌流量价值8-15万/年）"}],
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Long-Tail-Opportunity-Auto-Capture", "why": "长尾词机会自动捕获：AI持续监控高转化低竞争长尾词并自动追加投放，每月新增20-50个优质词（CPC降低35%）"}],
    ...
    },
    {
        ...
        "name": "差评防御作战手册",
        "tag": "评论 · 防御 · 口碑",
        "desc": "Amazon评分每降0.1星销量跌5-8%：从差评早期预警到根因修复的完整防御体系",
        "intro": "Amazon评分是决定搜索排名和转化率的核心因子。每下降0.1颗星，平均销量下降5-8%；跌破4.0星，FBA优选资格和搜索排名将受到直接影响。但大多数卖家的应对方式是「出了差评才处理」——这时已经晚了。本手册建立「预警→根因→防御→修复」的主动管理闭环，让差评影响在可见之前就被拦截。",
        "steps": [
            {
                "step": "Step 1 — 差评速率异常监控（持续）",
                "problem": "竞品恶意刷差评或产品真实问题爆发时，无法在早期识别，等评分下降才发现已损失惨重",
                "skills": [
                    {"id": "Skill-Review-Velocity-Anomaly-Detector", "why": "CUSUM算法实时监控差评速率，区分「恶意攻击」（突然涌入）vs「产品问题」（渐进增加），24小时内触发预警"},
                    {"id": "Skill-Fake-Review-Detection", "why": "识别疑似刷差评账号特征（账号龄/评论模式/验证购买比例），为平台申诉提供证据"},
                ],
                "data": "需要：Amazon后台评论数据（每日拉取）、评论账号元数据",
                "output": "差评速率监控看板 + 异常事件告警 + 恶意攻击证据包",
            },
            {
                "step": "Step 2 — 差评根因分析（事件触发）",
                "problem": "收到批量差评时，不知道是产品质量、物流损坏、用户误操作还是竞品攻击，无法针对性处理",
                "skills": [
                    {"id": "Skill-Negative-Review-Root-Cause-Analyzer", "why": "ABSA方面级情感分析将差评归因到「产品缺陷/物流损坏/使用不当/虚假期望」四类，输出优先级修复清单"},
                    {"id": "Skill-VOC-Product-Iteration-Signal-Extractor", "why": "从差评中提取具体改进方向，输出可直接交工厂的产品改良规格书"},
                ],
                "data": "需要：差评文本（近30天）、产品规格说明",
                "output": "根因分类报告（P0/P1/P2优先级）+ 产品改进清单 + 供应商沟通文档",
            },
            {
                "step": "Step 3 — 主动防御（Vine计划优化）",
                "problem": "知道有差评风险但不知道如何通过Vine计划系统性建立正向评分缓冲，投入ROI不清晰",
                "skills": [
                    {"id": "Skill-Review-Defense-Vine-Optimizer", "why": "计算当前评分分布下最优Vine投入量：需要多少个5星才能抵消N个1星的影响，基于贝叶斯更新预测ROI"},
                    {"id": "Skill-VOC-NPS-Retention-Predictor", "why": "通过评论语言特征预测用户满意度趋势，提前3-4周预警评分下降风险"},
                ],
                "data": "需要：当前评分分布、Vine计划成本、目标评分",
                "output": "Vine计划投入方案 + 预期评分恢复时间线 + ROI估算",
            },
            {
                "step": "Step 4 — 平台申诉与品牌保护（事后）",
                "problem": "面对恶意差评如何向Amazon有效申诉，以及如何防止品牌词被竞品利用",
                "skills": [
                    {"id": "Skill-Amazon-Account-Appeal-Strategy", "why": "基于证据链构建申诉材料：恶意刷评证据+账号异常分析+申诉信模板"},
                    {"id": "Skill-Brand-Defense-Search-Strategy", "why": "品牌词防守：防止竞品在品牌词搜索下截流，同时保持自然排名优势"},
                ],
                "data": "需要：Step1的恶意攻击证据包、品牌词列表",
                "output": "Amazon申诉材料包 + 品牌词防守广告策略",
            },
        ],
        "outcomes": [
            "差评预警响应时间从「次日发现」→「2小时告警」",
            "恶意差评申诉成功率提升40%（证据链完整）",
            "Vine计划ROI明确化，提升0.2星评分的成本降低30%",
            "产品改进周期从「客诉爆发→3个月改版」→「早期信号→6周迭代」",
        ],
    },
    {
        ...
        "name": "汇率关税风险对冲手册",
        "tag": "汇率 · 关税 · 财务风控",
        "desc": "人民币汇率±5-8%波动+301关税25%：跨境电商财务风险的量化对冲体系",
        "intro": "跨境电商面临两大隐性财务杀手：汇率波动和关税政策变化。以年GMV 5000万美元计，人民币升值3%直接损失净利润约150万元；一个HS码申报错误可能导致多缴关税200万元/年。但大多数卖家的「风险管理」是赌汇率、碰关税。本手册提供系统性的量化风险框架，将不确定性转化为可管理的已知风险。",
        "steps": [
            {
                "step": "Step 1 — 外汇敞口测量（季度执行）",
                "problem": "不知道自己有多大的汇率风险，无法回答「人民币升值3%，我损失多少」",
                "skills": [
                    {"id": "Skill-FX-Exposure-Measurement", "why": "计算净外汇敞口：美元应收-美元成本，分币种统计，量化单位汇率变动的利润影响"},
                    {"id": "Skill-FX-Natural-Hedging-Strategy", "why": "通过匹配外币收入和外币成本降低净敞口：在美国采购部分零件对冲美元收入"},
                ],
                "data": "需要：各市场收入（分币种）、成本结构（分币种）、应收账款期限",
                "output": "外汇敞口报告 + 自然对冲可行性分析 + 净敞口降低方案",
            },
            {
                "step": "Step 2 — 关税分类优化（新品上架前执行）",
                "problem": "同一产品不同HS码申报，关税率可能相差10-15个百分点，年化差异数百万",
                "skills": [
                    {"id": "Skill-HTS-Tariff-Classification", "why": "基于产品功能和材质，识别最优合规HS码分类，避免申报错误导致多缴税或合规风险"},
                    {"id": "Skill-MonteCarlo-Tariff-Risk", "why": "Monte Carlo模拟关税政策变化情景：如果Section 301再加10%，哪些品类利润会归零"},
                ],
                "data": "需要：产品技术规格、目标市场、历史进口申报记录",
                "output": "最优HS码建议 + 关税风险评分 + 多市场税负对比表",
            },
            {
                "step": "Step 3 — 定价汇率联动（持续执行）",
                "problem": "汇率变化了但定价没有及时调整，导致某些市场利润率大幅偏离目标",
                "skills": [
                    {"id": "Skill-FX-Dynamic-Pricing-Adjustment", "why": "建立「汇率变动→定价调整」联动规则：EUR/USD变动>2%时自动重算欧洲站价格，保持目标毛利率"},
                    {"id": "Skill-Elasticity-Based-Repricing-Gate", "why": "调价前先验证价格弹性：若弹性>1.5，小幅提价会导致销量大幅下降，需权衡汇率保护vs销量损失"},
                ],
                "data": "需要：各市场实时汇率、目标毛利率、价格弹性历史数据",
                "output": "汇率-定价联动规则表 + 自动触发阈值 + 影响范围预测",
            },
            {
                "step": "Step 4 — 风险压测与应急预案（季度执行）",
                "problem": "不知道极端情景（人民币升值10%+关税再加10%）下能否保持盈利",
                "skills": [
                    {"id": "Skill-MonteCarlo-Tariff-Risk", "why": "构建多情景压测：汇率±5%/±10% × 关税±5%/±10%，输出各情景下的利润矩阵和保本线"},
                    {"id": "Skill-Forecast-to-PL-Bridge", "why": "将风险情景映射到P&L：让CFO看懂「人民币升值5%=每月净利润减少X万」"},
                ],
                "data": "需要：当前定价结构、成本结构、目标利润率",
                "output": "风险压测矩阵 + 应急预案 + 董事会汇报版本",
            },
        ],
        "outcomes": [
            "外汇净敞口降低40-60%（自然对冲实施后）",
            "关税分类优化节省年化50-200万元",
            "汇率波动对利润率的冲击从±3pp降至±1pp",
            "极端风险场景提前准备，避免被动应对",
        ],
    },
    {
        ...
        "steps": [
            ...
            },
            {
                ...
                "skills": [
                    ...
                    {"id": "Skill-Search-Rank-Recovery-Auto-Action", "why": "ASIN排名恢复自动化行动：排名下滑触发自动诊断→优化Listing→调整广告→监控恢复，平均恢复周期从14天→5天"}],
